...
>
> **⚡ NEW ENGINE RENDERING (garry's UI rendering update, noted 2026-09-01 — VERIFY the installed build has it before relying on it):** `border-shape: polygon( x% y%, … )` (up to 8 vertices, concave via even-odd) and `border-shape: circle( r at x y )` render as SDF shapes — anti-aliased, take `border`/`filter: drop-shadow`, and HIT-TEST on the real edge. **This is the angle grammar WITHOUT `transform: skewX`** — a skewed chip/seam/stamp becomes a plain box with a shape: no scroll-clip leak, no flex-grow poisoning, no counter-skew spans, no ring skew compensation. Migration candidate for every shared chip/seam/stamp recipe. Also: corner radii UNIFIED across box/border/clip/shadow (+ h/v radii, pill clamp past half-height, `%` radii) — re-verify then retire the 'radius > half box clips to nothing' law; UI now blends in GAMMA mode (alpha reads like the web — re-check tuned rgba ladders); `background-clip: text` (gradient/image text — ration hard, house titles stay flat cream).
>
