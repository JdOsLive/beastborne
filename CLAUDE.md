...
| **`border-left` / `border-right` / etc. shorthand with style word fails** | s&box rejects `border-left: 3px solid rgba(...)` — logs "solid is not valid with border-style". The per-side border shorthand can't include the style keyword. Workarounds: (1) use the full uniform `border: 1px solid rgba(...)` shorthand (all four sides), (2) use a nested absolutely-positioned div for asymmetric accent strips (see `.item-rarity-bar` in InventoryPanel), (3) simulate a left accent via `box-shadow: -3px 0 0 rgba(...)` (outer shadow, no inset). |
...
| **`radial-gradient(ellipse, ...)` rejected — use `circle`** | Runtime log: "Cannot read a color from 'ellipse at X% Y%'" and similar. The `ellipse` shape word (with or without positional args) is not parsed. Use `radial-gradient(circle, ...)` with optional size/position args. For non-square glows, accept the circular shape — it rarely matters visually at typical alpha levels. |
| **`object-position` with percentage pairs rejected — use keyword values** | Runtime log: "Generic 50% 25% is not valid with object-position". The parser accepts keyword values (`center top`, `left bottom`, etc.) but not percentage pairs. Workaround: stick to keyword values, OR drop `object-position` entirely and rely on `object-fit: contain` default centering. |
