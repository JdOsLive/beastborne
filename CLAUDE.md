...
| **`object-position` REJECTS BOTH percentage pairs AND keyword pairs — basically unusable** | Runtime log: both "Generic 50% 25% is not valid with object-position" AND "Generic center bottom is not valid with object-position" (verified 2026-05-01). Two-value forms always fail, regardless of whether the parts are percentages or keywords. Workaround: drop `object-position` entirely and rely on `object-fit: contain`'s default centering. To bottom-anchor a contained image, use a parent `display: flex; align-items: flex-end;` and compensate the inner vertical-centering via a per-element transform-Y offset on the wrapper (the image will always center in its slot regardless of where the slot sits). |
