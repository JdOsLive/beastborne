...
### BeastiaryPanel (user-facing name: "BEASTBOOK") _[MUSEUM-WALL structural rebuild + teal accent + living ring, 2026-07-02 sweep]_
- **Files:** `Code/UI/Panels/BeastiaryPanel.razor` (~2200), `.scss` (~3300)
- **Purpose:** Species catalog / "book of bound beasts." **2026-07-02 sweep: rebuilt as THE MUSEUM WALL** — three zones: INDEX RAIL (left, 384px: completion hero stat head + 76px sprite-thumb slot grid, 4 cols) | SHOWCASE STAGE (center: ONE species giant — 420px animated sprite, teal radial pedestal glow, skewed teal plinth, 110px registry-number watermark behind) | PLACARD (right, 460px gallery label: identity, flavor, diamond actions, swappable views). Accent = Beastbook teal `#2dd4bf` (kicker, seams, header signature strip, section-title skew slabs, active diamonds, completion bar, evo connectors); violet `#9b6cff` reserved for the cursor.
