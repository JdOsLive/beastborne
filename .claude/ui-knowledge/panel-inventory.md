...
### PhoneLauncher _[new 2026-07-01, v2 same day from live feedback — experimental, A/B alongside the command bar]_
- **Files:** `Code/UI/Components/PhoneLauncher.razor` (~490), `.scss` (~360)
- **Purpose:** Phone-shaped nav LAUNCHER (guiding-star §Navigation) — a 3×4 grid of accent-colored "app" icons, one per destination. A router, not content. Summoned by Slot8 / the MENU bar button; the bottom bar stays until the user blesses retirement.
- **Visual style notes (v2):** sibling of the main menu — fullscreen dim backdrop (`top/left: 0 + width/height: 100%` — all-edge stretching renders zero-size on an empty element) → `.pl-wrap` (2D entrance holder, translateY 30→0 + fade, glide ease) → `.pl-device` (`perspective(2000px) rotateY(6deg)` lean, NO bg — 3D holder rule) → `.pl-surface` (540px content width, RSlab-30 dark slab gradient, padding 26/34/24/34 — a substantial device on the 1920 canvas). Chrome: clock (15px) + tamer LV (14px) strip, BbSectionHeader size-28 (kicker TAMER-LINK / italic "Launcher" / violet accent 52→104), tilted violet BbStamp PROTOTYPE seated at `margin-top: 22px` so it sits beside the TITLE line (not crowding the clock/LV strip), WASD·ENTER·Q key-cap hints.
- **App icons (v2 — "apps look like apps"):** each app = an 88px rounded square (radius 22) FILLED SOLID with its accent — the color IS the icon; the launcher grid is the ONE sanctioned full-rainbow surface. 40px white glyph (dark ink `#2a1605` on gold/electric via DarkGlyph), 13px/800/UPPER italic label BELOW the icon with key cap beside it. Tiles are transparent 168px hit targets (168×3 + 18×2 gaps = 540). Focus = concentric violet ring wrapping the ICON square (real bordered child, host 22 + 6 = 28px radius) + fill brightened 18% toward white via C# inline swap (NOT a white overlay child — absolute full-cover overlays paint BEHIND a parent's background-color, engine quirk). Press = glyph dips to scale 0.88 (icon square never transforms — it hosts the ring). Count badge = accent pill with 3px dark punch-out border overlapping the icon's top-right corner, inside tile bounds; BbStamp NEW on bag. BbDiamond is GONE from the grid — the diamond stays the ACTION shape elsewhere; apps are squares.
...
- **Engine compliance:** parks at `left: -10000px` when closed (never display-flip); class-toggle transitions for open/close; slab styles re-asserted every frame (the held-tween lesson); tile children `pointer-events: none`; explicit widths everywhere under the 3D-leaned ancestor (no flex-grow).
