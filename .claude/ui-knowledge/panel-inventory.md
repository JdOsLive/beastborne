...
- **⚠️ 2026-06 NOTE: the bullets above pre-date the Concept C launcher-hub rework** (sidebar + featured card + cards row, `lh-` classes, motion-doctrine v2) — see `guiding-star.md` + MEMORY `ui-redesign-state.md` for the current shape. **2026-06-09 additions:** (a) Roadmap / Options / Patch Notes are no longer centered popups — they live in a shared **right-edge slide-in shell** (`.lh-detail` 720px + `.lh-detail-scrim`, `lhd-` content classes, slides via `right` property transition NOT transform, staggered child reveal via transition-delay; content speaks the OptionsPanel slab dialect). Old `.roadmap-*` / `.patchnotes-*` popup SCSS deleted (~840 lines). (b) PLAY detonation rebuilt: flat-keyframe gold flash + 12px white-headed streak + 200ms gold ring slam + 0.90→1.07 squash-overshoot + full-screen `.lh-screen-pulse` gold acknowledge wash.
