...
### PhoneLauncher _[new 2026-07-01 — experimental, A/B alongside the command bar]_
- **Files:** `Code/UI/Components/PhoneLauncher.razor` (~500), `.scss` (~330)
- **Purpose:** Phone-shaped nav LAUNCHER (guiding-star §Navigation) — a 3×4 grid of accent-colored "app" tiles, one per destination. A router, not content. Summoned by Slot8 / the MENU bar button; the bottom bar stays until the user blesses retirement.
- **Visual style notes:** sibling of the main menu — fullscreen dim backdrop → `.pl-wrap` (2D entrance holder, translateY 30→0 + fade, glide ease) → `.pl-device` (`perspective(2000px) rotateY(6deg)` lean, NO bg — 3D holder rule) → `.pl-surface` (388px-wide RSlab-30 dark slab gradient, real drop shadow). Chrome: clock + tamer LV strip, BbSectionHeader (kicker TAMER-LINK / italic "Launcher" / violet accent), tilted violet BbStamp PROTOTYPE, WASD·ENTER·Q key-cap hints. Tiles: 120×96 filled slabs (#131019 → #1c1830 on focus), BbDiamond app gem (30px, per-app accent), key-cap + italic label, count badges (accent pills, inside bounds) + BbStamp NEW on bag. Focus = concentric violet ring child (host 16 + 5 = 21px radius), hover==focus.
- **Accent map (one per destination — drives gem + badge + liquid):** Beasts `#7b4ddb` · Skills `#ff6bd6` · Expedition `#ee5421` · Online `#3f8fe0` · Beastbook `#2dd4bf` · Shop `#ffce3a` (dark glyph) · Quests `#3fb45e` · Bag `#d9a054` · Chat `#4aa8ff` · Radio `#c26bff` · Effects `#f7e024` (dark glyph) · Alerts `#e0414a`.
- **THE LIQUID EXPANSION (locked signature):** solid accent circle (100px, border-radius 50%) blooms from the tapped tile via per-frame `PanelTransform.AddScale`: Expand 240ms ease-out-cubic → route at u=0.5 (~87% coverage; the destination's mount-stall hides behind the flood; phone body hides same instant, modal releases) → Fade 180ms at full cover. 420ms total. Liquid for full-screen destinations only (6 tabs + Quests + Bag); widget apps close-and-toggle (juice-tier discipline). Root z-index 9500 so the circle covers QuestPanel (1000) / InventoryPanel (9000).
- **Engine compliance:** parks at `left: -10000px` when closed (never display-flip); class-toggle transitions for open/close; circle styles re-asserted every frame (rebuild-proof); tile children `pointer-events: none`; tiles found by fresh `HasClass("pl-app")` traversal (no stored refs).
...
- **Known issues / notes:** no P hotkey (no such input action; Slot8 is the summon). Bloom origin uses Box.Rect screen px as style px (MainMenu cursor precedent) — verify origin accuracy at non-1080p. Circle color is the raw accent at full saturation — if the gold/electric floods read too loud in-engine, blend accent ~55% toward `#0c0a18`. Launching a tab while a live expedition runs shows the leave-confirm AFTER the flood (coherent but slightly odd — revisit if it bothers).
