...
- 2026-09-25 — **Regional Hard tokens (Tide/Loom/Dawn/Threaded) removed** (user; unreleased, no sink, Tide un-earnable). Replaced by `HARD_MODE_FIRST_CLEAR_TOKENS` = +5 Tokens on each expedition's first Hard clear.
- 2026-09-25 — **Economy fixes A1–A4 applied** (user; `currency-audit-2026-09.md`): arena quests not generated while `MissionManager.RankedEnabled` is false (and pre-existing ones don't block bonuses); Token Collector rounds UP; boss-shop Ink×5 removed; repeat boss tokens Normal 1→2, Elite 2→3 (~100 Tokens/week typical).
