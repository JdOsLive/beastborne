...
public class SkillEffect
{
	...
	public string GetDescription()
	{
		...
		return Type switch
		{
			...
			SkillEffectType.GeneBonusFlat => $"{valueStr} to every gene on fusion",
			...
			SkillEffectType.RareItemChance => $"{valueStr}% rare drop weighting",
			SkillEffectType.TeamSizeBonus => $"{valueStr} team size",
			SkillEffectType.EncounterRateBonus => $"{valueStr}% encounter rate (extra wild beast chance on light waves)",
