...
public sealed class MonsterManager : Component
{
	...
	private void SaveMaxMonsters()
	{
		...
		if ( blob == null ) return;
		blob.MaxMonsters = MaxMonsters;
		SaveService.Instance.MarkDirty( "monsters" );
	}
	...
	// ═════════════════════════════════════════════════════════════════════
	// LAUNCH ROSTER — canonical list of species that ship at launch.
	// Single source of truth for the BeastiaryPanel filter, essence drops,
	// and anywhere else we want to hide the 113 AI-holdover species we
	// don't show players at launch. Reads: 4 handmade + 9 zone-1 wilds +
	// 9 zone-2 nature pool + 7 zone-3 water pool + 9 evolutions (3 zone-1
	// mid-stage, 3 starter mid-evos, 3 starter final evos). ~36 total.
	// ═════════════════════════════════════════════════════════════════════
	public static readonly HashSet<string> LaunchRoster = new()
	{
		...
		// Weaverwood — bigfoot shepherd line (handmade — Jet)
	...
	protected override void OnStart()
	{
		...
		{
			Log.Warning( "[MonsterManager] SaveService.Instance is null; starting with empty roster" );
			OwnedMonsters = new();
	...
	private void LoadSpeciesDatabase()
	{
		...
		// from each species and falls back to "{Name} Spirit" when unset.
		ItemManager.Instance?.RegisterBeastMaterials( _speciesDatabase, LaunchRoster );
	}
	...
	private void CreateDefaultSpecies()
	{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BaseHP = 48, BaseATK = 65, BaseDEF = 42, BaseSpA = 40, BaseSpD = 35, BaseSPD = 53,
			HPGrowth = 5, ATKGrowth = 6, DEFGrowth = 5, SpAGrowth = 5, SpDGrowth = 5, SPDGrowth = 6,
			...
			BaseCatchRate = 0.6f,
			BaseExpYield = 100,
			SignatureDropName = "Ember Ash",
			SignatureDropDescription = "Warm grey dust left where an Embrik's hollow flame burned itself cool.",
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "harden", LearnLevel = 12 },
				new LearnableMove { MoveId = "flame_tackle", LearnLevel = 13 },
				new LearnableMove { MoveId = "pyre_fangs", LearnLevel = 15 },
				new LearnableMove { MoveId = "singe_breath", LearnLevel = 17 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "ember_claw", LearnLevel = 1 },
				new LearnableMove { MoveId = "smolder", LearnLevel = 5 },
				...
				new LearnableMove { MoveId = "lava_plume", LearnLevel = 40 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			IconOffsetX = 8f,
			IconOffsetY = 26f,
			// Grid-card art: scale is already generous at 1.3x. Without an
			// offset, the head clips the top edge of the card. Push the
			// whole portrait 12px down so the helmet sits just under the
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BaseHP = 52, BaseATK = 37, BaseDEF = 52, BaseSpA = 43, BaseSpD = 55, BaseSPD = 44,
			HPGrowth = 6, ATKGrowth = 5, DEFGrowth = 6, SpAGrowth = 5, SpDGrowth = 6, SPDGrowth = 4,
			...
			BaseCatchRate = 0.6f,
			BaseExpYield = 100,
			SignatureDropName = "Knight's Fin",
			SignatureDropDescription = "A small salt-rimed fin engraved with the tide-marks of Pagefin's first patrol.",
			Personality = BeastPersonality.Loyal,
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "brine_spray", LearnLevel = 3 },
				new LearnableMove { MoveId = "aqua_shield", LearnLevel = 5 },
				new LearnableMove { MoveId = "drizzle_mist", LearnLevel = 7 },
				new LearnableMove { MoveId = "water_bolt", LearnLevel = 9 },
				new LearnableMove { MoveId = "aqua_strike", LearnLevel = 10 },
				new LearnableMove { MoveId = "froth_barrage", LearnLevel = 12 },
				new LearnableMove { MoveId = "foam_coat", LearnLevel = 14 },
				new LearnableMove { MoveId = "harden", LearnLevel = 15 },
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 17 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 18 },
				new LearnableMove { MoveId = "tidal_slam", LearnLevel = 20 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 21 },
				new LearnableMove { MoveId = "current_boost", LearnLevel = 22 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 23 },
				new LearnableMove { MoveId = "torrent_claw", LearnLevel = 24 },
				new LearnableMove { MoveId = "deep_pressure", LearnLevel = 25 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "water_bolt", LearnLevel = 5 },
				new LearnableMove { MoveId = "aqua_strike", LearnLevel = 8 },
				new LearnableMove { MoveId = "brine_spray", LearnLevel = 10 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 12 },
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 15 },
			...
			IconOffsetX = 10f,
			IconOffsetY = 24f,
			CardScale = 1.25f, CardOffsetX = 9f, CardOffsetY = 59f,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "harden", LearnLevel = 1 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 5 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 10 },
				new LearnableMove { MoveId = "whirlpool_surge", LearnLevel = 15 },
			...
			IconOffsetX = 10f,
			IconOffsetY = 32f,
			// Grid-card art: good silhouette but reads tight at 1.15x.
			// Bump scale a hair and nudge right + down so the sea-beast
			// fills more of the card without clipping the left edge.
			CardScale = 1.25f,
			CardOffsetX = 22f,
			CardOffsetY = 38f,
		...
		// ═══════════════════════════════════════════════════════════════
		// PROTECTOR STARTER LINE (#7-9): Cherune → Seraphiel → Aurael (stage 3 sprite + data pending)
		// ═══════════════════════════════════════════════════════════════
		...
		AddSpecies( new MonsterSpecies
		{
			Id = "cherune",
			Name = "Cherune",
			Description = "A novice cherub barely larger than the halo it wears. It hovers beside lost travelers for a few breaths and vanishes, leaving a warmth that feels like someone was watching over them.",
			IconPath = "ui/monsters/cherune/idle/cherune_idle_01.png",
			AnimationFrames = new()
			{
				"ui/monsters/cherune/idle/cherune_idle_01.png"
			},
			AnimationFrameRate = 6f,
			BaseHP = 40, BaseATK = 53, BaseDEF = 34, BaseSpA = 48, BaseSpD = 39, BaseSPD = 69,
			HPGrowth = 5, ATKGrowth = 5, DEFGrowth = 4, SpAGrowth = 5, SpDGrowth = 5, SPDGrowth = 8,
			...
			BaseCatchRate = 0.55f,
			BaseExpYield = 100,
			SignatureDropName = "Lost Feather",
			SignatureDropDescription = "A stray down feather left behind where a Cherune hovered too long.",
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "spirit_breeze", LearnLevel = 3 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 5 },
				new LearnableMove { MoveId = "gust_claw", LearnLevel = 7 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 8 },
				new LearnableMove { MoveId = "draft_rush", LearnLevel = 10 },
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 12 },
				new LearnableMove { MoveId = "calm_air", LearnLevel = 13 },
				new LearnableMove { MoveId = "updraft", LearnLevel = 15 },
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 17 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 18 },
				new LearnableMove { MoveId = "zephyr_bolt", LearnLevel = 19 },
				new LearnableMove { MoveId = "razor_gale", LearnLevel = 20 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 22 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 23 },
				new LearnableMove { MoveId = "tailwind_boost", LearnLevel = 24 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			AnimationFrames = new()
			{
				"ui/monsters/seraphiel/idle/seraphiel_idle_01.png"
			},
			AnimationFrameRate = 6f,
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 30 },
				new LearnableMove { MoveId = "gale_slam", LearnLevel = 34 },
				new LearnableMove { MoveId = "storm_talon", LearnLevel = 36 },
			...
			CardScale = 1.8f, CardOffsetX = 29f, CardOffsetY = 185f,
		...
		AddSpecies( new MonsterSpecies
		{
			Id = "aurael",
			Name = "Aurael",
			Description = "The crowned herald of the protector host. Every soul a Cherune once sheltered returns as a phantom hand at its side, and Aurael's verdict is always mercy — even when mercy requires a blade.",
			IconPath = "ui/monsters/aurael/idle/aurael_idle_01.png",
			AnimationFrames = new()
			{
				"ui/monsters/aurael/idle/aurael_idle_01.png"
			},
			AnimationFrameRate = 6f,
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "storm_talon", LearnLevel = 18 },
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 22 },
				...
				new LearnableMove { MoveId = "typhoon_wrath", LearnLevel = 55 },
			},
			BeastiaryNumber = 9,
			IconOffsetX = 14f,
			IconOffsetY = 32f,
			...
			CardScale = 1.95f, CardOffsetX = 10f, CardOffsetY = 162f,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BaseCatchRate = 0.7f,
			BaseExpYield = 55,
			SignatureDropName = "Snapped Twig",
			SignatureDropDescription = "A dry brown twig, freshly broken from a Twigsnap's wooden frame.",
			Personality = BeastPersonality.Timid,
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "bramble_strike", LearnLevel = 12 },
				new LearnableMove { MoveId = "brace", LearnLevel = 14 },
				new LearnableMove { MoveId = "root_bind", LearnLevel = 18 },
				new LearnableMove { MoveId = "wild_thorn", LearnLevel = 19 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 20 },
				new LearnableMove { MoveId = "seed_volley", LearnLevel = 22 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 23 },
				new LearnableMove { MoveId = "pollen_burst", LearnLevel = 24 },
				new LearnableMove { MoveId = "tangling_roots", LearnLevel = 25 },
				new LearnableMove { MoveId = "vine_crush", LearnLevel = 26 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 27 },
				new LearnableMove { MoveId = "thorn_barrage", LearnLevel = 28 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 30 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BaseCatchRate = 0.4f,
			BaseExpYield = 140,
			SignatureDropName = "Heartwood Branch",
			SignatureDropDescription = "A dense, rune-grained branch from the core of a Branchling. Warm to the touch.",
			Personality = BeastPersonality.Loyal,
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "bramble_strike", LearnLevel = 10 },
				...
				new LearnableMove { MoveId = "brace", LearnLevel = 24 },
				new LearnableMove { MoveId = "thorn_barrage", LearnLevel = 26 },
				new LearnableMove { MoveId = "root_bind", LearnLevel = 28 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 30 },
				new LearnableMove { MoveId = "timber_slam", LearnLevel = 32 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 33 },
				new LearnableMove { MoveId = "tangling_roots", LearnLevel = 34 },
				new LearnableMove { MoveId = "canopy_crash", LearnLevel = 36 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 37 },
				new LearnableMove { MoveId = "overgrowth_slam", LearnLevel = 38 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 39 },
		...
		AddSpecies( new MonsterSpecies
		{
			Id = "dewdrop",
			Name = "Dewdrop",
			Description = "A small round waterdrop with a single leaf for a cap. It bobs along still ponds at dawn and sleeps through the heat of the day.",
			...
			SignatureDropDescription = "A perfect globe of water that refuses to evaporate. Cool to the touch, sweet-smelling. A leaf-cap drifts off it after a few minutes.",
			Personality = BeastPersonality.Timid,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BaseCatchRate = 0.6f,
			BaseExpYield = 55,
			SignatureDropName = "Dust Tuft",
			SignatureDropDescription = "A shimmering wisp of fine Dustling fuzz. Smells faintly of starlight.",
			Personality = BeastPersonality.Timid,
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "kindle", LearnLevel = 1 },
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "smolder", LearnLevel = 3 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "singe_breath", LearnLevel = 7 },
				new LearnableMove { MoveId = "breeze_cut", LearnLevel = 8 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 10 },
				new LearnableMove { MoveId = "ember_claw", LearnLevel = 12 },
				new LearnableMove { MoveId = "heat_haze", LearnLevel = 14 },
				new LearnableMove { MoveId = "searing_rush", LearnLevel = 15 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 16 },
				new LearnableMove { MoveId = "cinders_curse", LearnLevel = 18 },
				new LearnableMove { MoveId = "scorch_wave", LearnLevel = 20 },
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 22 },
				new LearnableMove { MoveId = "molten_ray", LearnLevel = 23 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 24 },
				new LearnableMove { MoveId = "flame_vortex", LearnLevel = 25 },
				new LearnableMove { MoveId = "ash_cloud", LearnLevel = 25 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "razor_gale", LearnLevel = 1, EvolvesFrom = "breeze_cut" },
				new LearnableMove { MoveId = "ember_claw", LearnLevel = 3 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "singe_breath", LearnLevel = 8 },
				new LearnableMove { MoveId = "flame_vortex", LearnLevel = 10 },
				new LearnableMove { MoveId = "heat_haze", LearnLevel = 14 },
				new LearnableMove { MoveId = "rift_flame", LearnLevel = 16 },
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 18 },
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 20 },
				new LearnableMove { MoveId = "cinders_curse", LearnLevel = 24 },
				new LearnableMove { MoveId = "molten_ray", LearnLevel = 26 },
				new LearnableMove { MoveId = "ash_cloud", LearnLevel = 28 },
				new LearnableMove { MoveId = "scorching_gust", LearnLevel = 30 },
				new LearnableMove { MoveId = "blazing_wrath", LearnLevel = 32 },
				new LearnableMove { MoveId = "lava_plume", LearnLevel = 35 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 38 },
				new LearnableMove { MoveId = "wildfire_burst", LearnLevel = 40 },
				new LearnableMove { MoveId = "brace", LearnLevel = 42 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BaseCatchRate = 0.65f,
			BaseExpYield = 75,
			SignatureDropName = "Elder Moss",
			SignatureDropDescription = "A patch of ancient green felt scraped from a Mosscreep's back. Heavy for its size.",
			Personality = BeastPersonality.Timid,
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "gravel_toss", LearnLevel = 3 },
				new LearnableMove { MoveId = "harden", LearnLevel = 6 },
				new LearnableMove { MoveId = "sand_blast", LearnLevel = 8 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 10 },
				new LearnableMove { MoveId = "boulder_toss", LearnLevel = 12 },
				new LearnableMove { MoveId = "dust_storm", LearnLevel = 14 },
				new LearnableMove { MoveId = "earthrend", LearnLevel = 16 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 18 },
				new LearnableMove { MoveId = "temper", LearnLevel = 20 },
				new LearnableMove { MoveId = "bedrock_stance", LearnLevel = 21 },
				new LearnableMove { MoveId = "sandstone_strike", LearnLevel = 22 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 23 },
				new LearnableMove { MoveId = "quicksand", LearnLevel = 24 },
				new LearnableMove { MoveId = "tremor_pulse", LearnLevel = 25 },
				new LearnableMove { MoveId = "ground_pulse", LearnLevel = 26 },
				new LearnableMove { MoveId = "geode_burst", LearnLevel = 28 },
				new LearnableMove { MoveId = "petrify", LearnLevel = 29 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BaseCatchRate = 0.55f,
			BaseExpYield = 55,
			SignatureDropName = "Whisker",
			SignatureDropDescription = "A fine silver whisker from a Whiskerwind. Still hums with half-caught whispers.",
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "breeze_cut", LearnLevel = 1 },
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "rend", LearnLevel = 3 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "gust_claw", LearnLevel = 7 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 9 },
				new LearnableMove { MoveId = "draft_rush", LearnLevel = 10 },
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 12 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 13 },
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 15 },
				new LearnableMove { MoveId = "gust_shield", LearnLevel = 16 },
				new LearnableMove { MoveId = "updraft", LearnLevel = 18 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 19 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 20 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 22 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 23 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 17 },
				new LearnableMove { MoveId = "storm_talon", LearnLevel = 20 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 22 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 25 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 28 },
				new LearnableMove { MoveId = "gale_fang", LearnLevel = 30 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 32 },
				new LearnableMove { MoveId = "tornado_dive", LearnLevel = 34 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 36 },
				new LearnableMove { MoveId = "tempest", LearnLevel = 38 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BaseCatchRate = 0.6f,
			BaseExpYield = 75,
			SignatureDropName = "Glimcap",
			SignatureDropDescription = "The luminous mushroom cap of a Glimshroom. Glows faintly in the dark.",
			Personality = BeastPersonality.Timid,
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "parasite_spore", LearnLevel = 17 },
				new LearnableMove { MoveId = "pollen_burst", LearnLevel = 19 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 20 },
				new LearnableMove { MoveId = "root_bind", LearnLevel = 22 },
				new LearnableMove { MoveId = "toxic_pollen", LearnLevel = 23 },
				new LearnableMove { MoveId = "brace", LearnLevel = 24 },
				new LearnableMove { MoveId = "bloom_burst", LearnLevel = 25 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 26 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 28 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 29 },
				new LearnableMove { MoveId = "growth_surge", LearnLevel = 30 },
				new LearnableMove { MoveId = "photon_bloom", LearnLevel = 32 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "smolder", LearnLevel = 7 },
				new LearnableMove { MoveId = "flame_tackle", LearnLevel = 9 },
				new LearnableMove { MoveId = "harden", LearnLevel = 10 },
				new LearnableMove { MoveId = "pyre_fangs", LearnLevel = 12 },
				new LearnableMove { MoveId = "ember_claw", LearnLevel = 14 },
				new LearnableMove { MoveId = "singe_breath", LearnLevel = 16 },
				new LearnableMove { MoveId = "searing_rush", LearnLevel = 18 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 19 },
				new LearnableMove { MoveId = "magma_fang", LearnLevel = 20 },
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 22 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 23 },
				new LearnableMove { MoveId = "heat_haze", LearnLevel = 24 },
				new LearnableMove { MoveId = "scorch_wave", LearnLevel = 25 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 25 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "smolder", LearnLevel = 5 },
				new LearnableMove { MoveId = "harden", LearnLevel = 8 },
				new LearnableMove { MoveId = "magma_fang", LearnLevel = 14 },
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 18 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 20 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 24 },
				new LearnableMove { MoveId = "eruption_slam", LearnLevel = 26 },
				new LearnableMove { MoveId = "ash_cloud", LearnLevel = 28 },
				new LearnableMove { MoveId = "cinders_curse", LearnLevel = 30 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 32 },
				new LearnableMove { MoveId = "lava_plume", LearnLevel = 34 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 36 },
				new LearnableMove { MoveId = "blazing_wrath", LearnLevel = 38 },
				new LearnableMove { MoveId = "volcanic_fury", LearnLevel = 40 },
				new LearnableMove { MoveId = "conflagration", LearnLevel = 42 },
				new LearnableMove { MoveId = "brace", LearnLevel = 44 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "smolder", LearnLevel = 4 },
				new LearnableMove { MoveId = "ember_claw", LearnLevel = 6 },
				new LearnableMove { MoveId = "harden", LearnLevel = 8 },
				new LearnableMove { MoveId = "flame_tackle", LearnLevel = 10 },
				new LearnableMove { MoveId = "forge_temper", LearnLevel = 12 },
				new LearnableMove { MoveId = "searing_rush", LearnLevel = 15 },
				new LearnableMove { MoveId = "ember_wall", LearnLevel = 18 },
				new LearnableMove { MoveId = "brace", LearnLevel = 20 },
				new LearnableMove { MoveId = "seismic_crash", LearnLevel = 22 },
				new LearnableMove { MoveId = "lava_plume", LearnLevel = 24 },
				new LearnableMove { MoveId = "eruption_slam", LearnLevel = 26 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 28 },
				new LearnableMove { MoveId = "blazing_wrath", LearnLevel = 30 },
				new LearnableMove { MoveId = "molten_ray", LearnLevel = 33 },
				new LearnableMove { MoveId = "volcanic_fury", LearnLevel = 36 },
				new LearnableMove { MoveId = "conflagration", LearnLevel = 38 },
				new LearnableMove { MoveId = "cinders_curse", LearnLevel = 40 },
				new LearnableMove { MoveId = "flame_vortex", LearnLevel = 42 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "intimidate", LearnLevel = 7 },
				new LearnableMove { MoveId = "ember_claw", LearnLevel = 8 },
				new LearnableMove { MoveId = "pyre_fangs", LearnLevel = 10 },
				new LearnableMove { MoveId = "flame_tackle", LearnLevel = 12 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 13 },
				new LearnableMove { MoveId = "singe_breath", LearnLevel = 14 },
				new LearnableMove { MoveId = "searing_rush", LearnLevel = 16 },
				new LearnableMove { MoveId = "harden", LearnLevel = 17 },
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 19 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 20 },
				new LearnableMove { MoveId = "heat_haze", LearnLevel = 22 },
				new LearnableMove { MoveId = "scorch_wave", LearnLevel = 23 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 24 },
				new LearnableMove { MoveId = "magma_fang", LearnLevel = 25 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "ember_claw", LearnLevel = 1 },
				new LearnableMove { MoveId = "smolder", LearnLevel = 5 },
				new LearnableMove { MoveId = "flame_tackle", LearnLevel = 8 },
				new LearnableMove { MoveId = "harden", LearnLevel = 12 },
				new LearnableMove { MoveId = "magma_fang", LearnLevel = 16 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 18 },
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 20 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 22 },
				new LearnableMove { MoveId = "scorch_wave", LearnLevel = 24 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 26 },
				new LearnableMove { MoveId = "cinders_curse", LearnLevel = 28 },
				new LearnableMove { MoveId = "eruption_slam", LearnLevel = 30 },
				new LearnableMove { MoveId = "ash_cloud", LearnLevel = 32 },
				new LearnableMove { MoveId = "blazing_wrath", LearnLevel = 34 },
				new LearnableMove { MoveId = "lava_plume", LearnLevel = 36 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 38 },
				new LearnableMove { MoveId = "volcanic_fury", LearnLevel = 40 },
			},
			AnimationFrames = new()
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "eruption_slam", LearnLevel = 20 },
				new LearnableMove { MoveId = "ash_cloud", LearnLevel = 25 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "lava_plume", LearnLevel = 32 },
				new LearnableMove { MoveId = "volcanic_fury", LearnLevel = 36 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 40 },
				new LearnableMove { MoveId = "flame_vortex", LearnLevel = 42 },
				new LearnableMove { MoveId = "primal_roar", LearnLevel = 44 },
				new LearnableMove { MoveId = "pyroclasm", LearnLevel = 46 },
				new LearnableMove { MoveId = "conflagration", LearnLevel = 50 },
				new LearnableMove { MoveId = "wildfire_burst", LearnLevel = 53 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 55 },
				new LearnableMove { MoveId = "primordial_flame", LearnLevel = 55 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "water_bolt", LearnLevel = 7 },
				new LearnableMove { MoveId = "harden", LearnLevel = 9 },
				...
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 19 },
				new LearnableMove { MoveId = "tidal_slam", LearnLevel = 20 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 21 },
				new LearnableMove { MoveId = "taunt_roar", LearnLevel = 22 },
				new LearnableMove { MoveId = "deep_pressure", LearnLevel = 23 },
				new LearnableMove { MoveId = "waterlog", LearnLevel = 24 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 25 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			Id = "mirrorpond",
			Name = "Mirrorpond",
			Description = "A creature that is the surface and depth of still water at once. Those who gaze into it see not themselves, but who they were afraid to become.",
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "aqua_shield", LearnLevel = 20 },
				new LearnableMove { MoveId = "deep_pressure", LearnLevel = 22 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 24 },
				new LearnableMove { MoveId = "tidal_slam", LearnLevel = 26 },
				new LearnableMove { MoveId = "whirlpool_surge", LearnLevel = 28 },
				new LearnableMove { MoveId = "brace", LearnLevel = 30 },
				new LearnableMove { MoveId = "crashing_wave", LearnLevel = 32 },
				new LearnableMove { MoveId = "deluge", LearnLevel = 34 },
				new LearnableMove { MoveId = "sea_chant", LearnLevel = 36 },
				new LearnableMove { MoveId = "ocean_torrent", LearnLevel = 38 },
				new LearnableMove { MoveId = "temper", LearnLevel = 40 },
				new LearnableMove { MoveId = "maelstrom_slam", LearnLevel = 42 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "drizzle_mist", LearnLevel = 3 },
				new LearnableMove { MoveId = "brine_spray", LearnLevel = 5 },
				new LearnableMove { MoveId = "water_bolt", LearnLevel = 7 },
				new LearnableMove { MoveId = "aqua_strike", LearnLevel = 8 },
				new LearnableMove { MoveId = "froth_barrage", LearnLevel = 10 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 12 },
				new LearnableMove { MoveId = "foam_coat", LearnLevel = 13 },
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 15 },
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 17 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 18 },
				new LearnableMove { MoveId = "current_boost", LearnLevel = 19 },
				new LearnableMove { MoveId = "torrent_claw", LearnLevel = 20 },
				new LearnableMove { MoveId = "tidal_slam", LearnLevel = 22 },
				new LearnableMove { MoveId = "whirlpool_surge", LearnLevel = 23 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 24 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 25 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "foam_coat", LearnLevel = 17 },
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 18 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 20 },
				new LearnableMove { MoveId = "current_boost", LearnLevel = 21 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 22 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 23 },
				new LearnableMove { MoveId = "torrent_claw", LearnLevel = 24 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "harden", LearnLevel = 8 },
				new LearnableMove { MoveId = "aqua_strike", LearnLevel = 10 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 14 },
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 18 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 20 },
				new LearnableMove { MoveId = "torrent_claw", LearnLevel = 22 },
				new LearnableMove { MoveId = "temper", LearnLevel = 24 },
				new LearnableMove { MoveId = "whirlpool_surge", LearnLevel = 26 },
				new LearnableMove { MoveId = "aqua_shield", LearnLevel = 28 },
				new LearnableMove { MoveId = "crashing_wave", LearnLevel = 30 },
				new LearnableMove { MoveId = "deluge", LearnLevel = 32 },
				new LearnableMove { MoveId = "sea_chant", LearnLevel = 34 },
				new LearnableMove { MoveId = "deep_pressure", LearnLevel = 36 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "temper", LearnLevel = 1 },
				new LearnableMove { MoveId = "aqua_shield", LearnLevel = 5 },
				new LearnableMove { MoveId = "torrent_claw", LearnLevel = 10 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 14 },
				new LearnableMove { MoveId = "crashing_wave", LearnLevel = 18 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 22 },
				new LearnableMove { MoveId = "whirlpool_surge", LearnLevel = 26 },
				new LearnableMove { MoveId = "brace", LearnLevel = 30 },
				new LearnableMove { MoveId = "ocean_torrent", LearnLevel = 34 },
				new LearnableMove { MoveId = "deep_pressure", LearnLevel = 36 },
				new LearnableMove { MoveId = "maelstrom_slam", LearnLevel = 38 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 40 },
				new LearnableMove { MoveId = "sea_chant", LearnLevel = 42 },
				new LearnableMove { MoveId = "hydro_fang", LearnLevel = 44 },
				new LearnableMove { MoveId = "monsoon_call", LearnLevel = 46 },
				new LearnableMove { MoveId = "tsunami_crush", LearnLevel = 48 },
				new LearnableMove { MoveId = "abyssal_wave", LearnLevel = 50 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "drizzle_mist", LearnLevel = 3 },
				new LearnableMove { MoveId = "foam_coat", LearnLevel = 5 },
				new LearnableMove { MoveId = "harden", LearnLevel = 6 },
				new LearnableMove { MoveId = "aqua_strike", LearnLevel = 8 },
				new LearnableMove { MoveId = "brine_spray", LearnLevel = 10 },
				new LearnableMove { MoveId = "froth_barrage", LearnLevel = 12 },
				new LearnableMove { MoveId = "water_bolt", LearnLevel = 14 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 16 },
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 18 },
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 19 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 20 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 21 },
				new LearnableMove { MoveId = "waterlog", LearnLevel = 22 },
				new LearnableMove { MoveId = "tidal_slam", LearnLevel = 23 },
		...
		AddSpecies( new MonsterSpecies
		{
			Id = "foamwraith",
			Name = "Foamwraith",
			Description = "A ghostly jellyfish that drifts where sailors were lost at sea. It carries their final prayers in its trailing tendrils and guides ships away from the same fate.",
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 5 },
				new LearnableMove { MoveId = "water_bolt", LearnLevel = 8 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 12 },
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 16 },
				new LearnableMove { MoveId = "deep_pressure", LearnLevel = 20 },
				new LearnableMove { MoveId = "whirlpool_surge", LearnLevel = 24 },
				new LearnableMove { MoveId = "brace", LearnLevel = 26 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 28 },
				new LearnableMove { MoveId = "aqua_shield", LearnLevel = 30 },
				new LearnableMove { MoveId = "crashing_wave", LearnLevel = 32 },
				new LearnableMove { MoveId = "sea_chant", LearnLevel = 34 },
				new LearnableMove { MoveId = "deluge", LearnLevel = 36 },
				new LearnableMove { MoveId = "ocean_torrent", LearnLevel = 38 },
				new LearnableMove { MoveId = "undertow_strike", LearnLevel = 40 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "splash_jet", LearnLevel = 1 },
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "foam_coat", LearnLevel = 4 },
				new LearnableMove { MoveId = "harden", LearnLevel = 6 },
				new LearnableMove { MoveId = "brine_spray", LearnLevel = 8 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 10 },
				new LearnableMove { MoveId = "waterlog", LearnLevel = 12 },
				new LearnableMove { MoveId = "froth_barrage", LearnLevel = 14 },
				new LearnableMove { MoveId = "tidal_ward", LearnLevel = 16 },
				new LearnableMove { MoveId = "aqua_shield", LearnLevel = 18 },
				new LearnableMove { MoveId = "deep_pressure", LearnLevel = 20 },
				new LearnableMove { MoveId = "temper", LearnLevel = 22 },
				new LearnableMove { MoveId = "tidal_slam", LearnLevel = 24 },
				new LearnableMove { MoveId = "brace", LearnLevel = 26 },
				new LearnableMove { MoveId = "crashing_wave", LearnLevel = 28 },
				new LearnableMove { MoveId = "sea_chant", LearnLevel = 30 },
				new LearnableMove { MoveId = "deluge", LearnLevel = 32 },
				new LearnableMove { MoveId = "maelstrom_slam", LearnLevel = 36 },
				new LearnableMove { MoveId = "ocean_torrent", LearnLevel = 40 },
				new LearnableMove { MoveId = "monsoon_call", LearnLevel = 44 },
			},
			BeastiaryNumber = 0
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "breeze_cut", LearnLevel = 1 },
				new LearnableMove { MoveId = "rend", LearnLevel = 1 },
				new LearnableMove { MoveId = "gust_claw", LearnLevel = 3 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 6 },
				new LearnableMove { MoveId = "draft_rush", LearnLevel = 8 },
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 10 },
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 12 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 13 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 14 },
				new LearnableMove { MoveId = "razor_gale", LearnLevel = 16 },
				new LearnableMove { MoveId = "gust_shield", LearnLevel = 17 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 19 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 20 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 22 },
				new LearnableMove { MoveId = "updraft", LearnLevel = 23 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "gust_claw", LearnLevel = 5 },
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 8 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 10 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 13 },
				new LearnableMove { MoveId = "storm_talon", LearnLevel = 16 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 19 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 22 },
				new LearnableMove { MoveId = "gale_slam", LearnLevel = 22 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 25 },
				new LearnableMove { MoveId = "updraft", LearnLevel = 28 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 30 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 32 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 34 },
				new LearnableMove { MoveId = "tornado_dive", LearnLevel = 36 },
				new LearnableMove { MoveId = "tempest", LearnLevel = 38 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "breeze_cut", LearnLevel = 1 },
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 4 },
				new LearnableMove { MoveId = "gust_claw", LearnLevel = 6 },
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 8 },
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 10 },
				new LearnableMove { MoveId = "phase_shift", LearnLevel = 12 },
				new LearnableMove { MoveId = "razor_gale", LearnLevel = 15 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 17 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 19 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 22 },
				new LearnableMove { MoveId = "vortex_blast", LearnLevel = 24 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 26 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 28 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 30 },
				new LearnableMove { MoveId = "tempest", LearnLevel = 32 },
				new LearnableMove { MoveId = "wind_wall", LearnLevel = 34 },
				new LearnableMove { MoveId = "squall_burst", LearnLevel = 36 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 3 },
				new LearnableMove { MoveId = "gust_claw", LearnLevel = 5 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 6 },
				new LearnableMove { MoveId = "updraft", LearnLevel = 8 },
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 10 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 11 },
				new LearnableMove { MoveId = "draft_rush", LearnLevel = 13 },
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 14 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 16 },
				new LearnableMove { MoveId = "gust_shield", LearnLevel = 17 },
				new LearnableMove { MoveId = "razor_gale", LearnLevel = 18 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 20 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 21 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 23 },
				new LearnableMove { MoveId = "vortex_blast", LearnLevel = 24 },
				new LearnableMove { MoveId = "tailwind_boost", LearnLevel = 25 },
			},
			BeastiaryNumber = 0
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 16 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 19 },
				new LearnableMove { MoveId = "storm_talon", LearnLevel = 22 },
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 24 },
				new LearnableMove { MoveId = "vortex_blast", LearnLevel = 26 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 28 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 30 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 32 },
				new LearnableMove { MoveId = "tempest", LearnLevel = 34 },
				new LearnableMove { MoveId = "tornado_dive", LearnLevel = 36 },
				new LearnableMove { MoveId = "windshear", LearnLevel = 38 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 12 },
				new LearnableMove { MoveId = "storm_talon", LearnLevel = 16 },
				new LearnableMove { MoveId = "vortex_blast", LearnLevel = 20 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 24 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 28 },
				new LearnableMove { MoveId = "gale_slam", LearnLevel = 30 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 33 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 36 },
				new LearnableMove { MoveId = "hurricane_slam", LearnLevel = 39 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 42 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 45 },
				new LearnableMove { MoveId = "typhoon_wrath", LearnLevel = 48 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 10 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 11 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 13 },
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 14 },
				new LearnableMove { MoveId = "draft_rush", LearnLevel = 16 },
				new LearnableMove { MoveId = "gust_shield", LearnLevel = 17 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 18 },
				new LearnableMove { MoveId = "razor_gale", LearnLevel = 20 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 21 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 23 },
				new LearnableMove { MoveId = "vortex_blast", LearnLevel = 24 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 13 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 16 },
				new LearnableMove { MoveId = "storm_talon", LearnLevel = 19 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 22 },
				new LearnableMove { MoveId = "gale_slam", LearnLevel = 24 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 26 },
				new LearnableMove { MoveId = "vortex_blast", LearnLevel = 28 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 30 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 32 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 34 },
				new LearnableMove { MoveId = "tempest", LearnLevel = 36 },
				new LearnableMove { MoveId = "hurricane_slam", LearnLevel = 38 },
				new LearnableMove { MoveId = "windshear", LearnLevel = 40 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "breeze_cut", LearnLevel = 1 },
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "gust_claw", LearnLevel = 4 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 6 },
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 8 },
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 10 },
				new LearnableMove { MoveId = "draft_rush", LearnLevel = 12 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 14 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 16 },
				new LearnableMove { MoveId = "razor_gale", LearnLevel = 18 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 20 },
				new LearnableMove { MoveId = "vortex_blast", LearnLevel = 22 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 24 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 26 },
				new LearnableMove { MoveId = "tempest", LearnLevel = 28 },
				new LearnableMove { MoveId = "wind_wall", LearnLevel = 30 },
				new LearnableMove { MoveId = "squall_burst", LearnLevel = 32 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "spark_claw", LearnLevel = 3 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "nerve_lock", LearnLevel = 6 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 8 },
				new LearnableMove { MoveId = "spark_wave", LearnLevel = 10 },
				new LearnableMove { MoveId = "volt_charge", LearnLevel = 12 },
				new LearnableMove { MoveId = "charge_up", LearnLevel = 14 },
				new LearnableMove { MoveId = "stun_wave", LearnLevel = 16 },
				new LearnableMove { MoveId = "arc_bolt", LearnLevel = 18 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 19 },
				new LearnableMove { MoveId = "static_field", LearnLevel = 20 },
				new LearnableMove { MoveId = "bolt_rush", LearnLevel = 22 },
				new LearnableMove { MoveId = "shock_tackle", LearnLevel = 23 },
				new LearnableMove { MoveId = "harden", LearnLevel = 24 },
				new LearnableMove { MoveId = "voltage_ray", LearnLevel = 25 },
				new LearnableMove { MoveId = "energy_pulse", LearnLevel = 26 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "spark_wave", LearnLevel = 5 },
				new LearnableMove { MoveId = "charge_up", LearnLevel = 8 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 10 },
				new LearnableMove { MoveId = "bolt_rush", LearnLevel = 14 },
				new LearnableMove { MoveId = "stun_wave", LearnLevel = 18 },
				new LearnableMove { MoveId = "arc_bolt", LearnLevel = 22 },
				new LearnableMove { MoveId = "static_field", LearnLevel = 24 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 26 },
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 28 },
				new LearnableMove { MoveId = "plasma_charge", LearnLevel = 30 },
				new LearnableMove { MoveId = "chain_lightning", LearnLevel = 32 },
				new LearnableMove { MoveId = "surge_field", LearnLevel = 34 },
				new LearnableMove { MoveId = "brace", LearnLevel = 36 },
				new LearnableMove { MoveId = "overload_crash", LearnLevel = 38 },
				new LearnableMove { MoveId = "discharge_pulse", LearnLevel = 40 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "nerve_lock", LearnLevel = 1 },
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 1 },
				new LearnableMove { MoveId = "chain_lightning", LearnLevel = 5 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 10 },
				new LearnableMove { MoveId = "plasma_charge", LearnLevel = 15 },
				new LearnableMove { MoveId = "surge_field", LearnLevel = 20 },
				new LearnableMove { MoveId = "overcharge", LearnLevel = 25 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "quantum_flux", LearnLevel = 30 },
				new LearnableMove { MoveId = "thunder_fang", LearnLevel = 35 },
				new LearnableMove { MoveId = "storm_strike", LearnLevel = 40 },
				new LearnableMove { MoveId = "megavolt", LearnLevel = 43 },
				new LearnableMove { MoveId = "galvanic_shield", LearnLevel = 45 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 48 },
				new LearnableMove { MoveId = "ion_cannon", LearnLevel = 50 },
				new LearnableMove { MoveId = "storm_surge", LearnLevel = 52 },
				new LearnableMove { MoveId = "dimensional_rift", LearnLevel = 54 },
		...
		AddSpecies( new MonsterSpecies
		{
			Id = "staiju",
			Name = "Staiju",
			Description = "A sleek dark cat that crackles with cyan lightning whenever it moves. It sleeps in tall trees during clear weather and prowls the air during thunderstorms.",
			IconPath = "ui/monsters/staiju/idle/staiju_idle_01.png",
			ArtistCredit = "Jet",
			...
			BaseCatchRate = 0.5f,
			BaseExpYield = 75,
			SignatureDropName = "Storm Whisker",
			SignatureDropDescription = "A single whisker shed by a Staiju during storm season. It pulses with faint static even days after — hold it near a lantern and the flame leans toward it.",
			Personality = BeastPersonality.Wild,
			PersonalityHint = "It watches you from a high branch with cyan eyes that don't blink. When the thunder rolls in close, it stops watching and starts hunting.",
			PossibleTraits = new() { "static_charge", "trickster", "barbed_hide" },
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "static_jolt", LearnLevel = 1 },
				new LearnableMove { MoveId = "rend", LearnLevel = 1 },
				new LearnableMove { MoveId = "spark_claw", LearnLevel = 4 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 6 },
				new LearnableMove { MoveId = "lightning_fang", LearnLevel = 8 },
				new LearnableMove { MoveId = "stun_wave", LearnLevel = 10 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 12 },
				new LearnableMove { MoveId = "volt_charge", LearnLevel = 14 },
				new LearnableMove { MoveId = "charge_up", LearnLevel = 16 },
				new LearnableMove { MoveId = "static_field", LearnLevel = 18 },
				new LearnableMove { MoveId = "spark_wave", LearnLevel = 20 },
				new LearnableMove { MoveId = "storm_talon", LearnLevel = 22 },
				new LearnableMove { MoveId = "arc_bolt", LearnLevel = 24 },
				new LearnableMove { MoveId = "bolt_rush", LearnLevel = 26 },
				new LearnableMove { MoveId = "shock_tackle", LearnLevel = 27 },
				new LearnableMove { MoveId = "voltage_ray", LearnLevel = 29 },
				new LearnableMove { MoveId = "overcharge", LearnLevel = 30 },
				new LearnableMove { MoveId = "skyfall_pounce", LearnLevel = 32 }
			},
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "spark_claw", LearnLevel = 3 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 7 },
				new LearnableMove { MoveId = "stun_wave", LearnLevel = 9 },
				new LearnableMove { MoveId = "charge_up", LearnLevel = 10 },
				new LearnableMove { MoveId = "volt_charge", LearnLevel = 12 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 14 },
				new LearnableMove { MoveId = "spark_wave", LearnLevel = 16 },
				new LearnableMove { MoveId = "nerve_lock", LearnLevel = 18 },
				new LearnableMove { MoveId = "shock_tackle", LearnLevel = 19 },
				new LearnableMove { MoveId = "harden", LearnLevel = 20 },
				new LearnableMove { MoveId = "lightning_fang", LearnLevel = 21 },
				new LearnableMove { MoveId = "bolt_rush", LearnLevel = 22 },
				new LearnableMove { MoveId = "static_field", LearnLevel = 23 },
				new LearnableMove { MoveId = "voltage_ray", LearnLevel = 24 },
				new LearnableMove { MoveId = "energy_pulse", LearnLevel = 25 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "spark_wave", LearnLevel = 5 },
				new LearnableMove { MoveId = "charge_up", LearnLevel = 8 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 12 },
				new LearnableMove { MoveId = "bolt_rush", LearnLevel = 16 },
				new LearnableMove { MoveId = "stun_wave", LearnLevel = 20 },
				new LearnableMove { MoveId = "lightning_fang", LearnLevel = 24 },
				new LearnableMove { MoveId = "arc_bolt", LearnLevel = 28 },
				new LearnableMove { MoveId = "thunder_fang", LearnLevel = 28 },
				new LearnableMove { MoveId = "static_field", LearnLevel = 30 },
				new LearnableMove { MoveId = "overcharge", LearnLevel = 32 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 34 },
				new LearnableMove { MoveId = "plasma_charge", LearnLevel = 36 },
				new LearnableMove { MoveId = "brace", LearnLevel = 38 },
				new LearnableMove { MoveId = "chain_lightning", LearnLevel = 40 },
				new LearnableMove { MoveId = "overload_crash", LearnLevel = 42 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "spark_claw", LearnLevel = 4 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 6 },
				new LearnableMove { MoveId = "stun_wave", LearnLevel = 8 },
				new LearnableMove { MoveId = "volt_charge", LearnLevel = 10 },
				new LearnableMove { MoveId = "charge_up", LearnLevel = 12 },
				new LearnableMove { MoveId = "harden", LearnLevel = 14 },
				new LearnableMove { MoveId = "froth_barrage", LearnLevel = 16 },
				new LearnableMove { MoveId = "spark_wave", LearnLevel = 18 },
				new LearnableMove { MoveId = "nerve_lock", LearnLevel = 20 },
				new LearnableMove { MoveId = "bolt_rush", LearnLevel = 22 },
				new LearnableMove { MoveId = "arc_bolt", LearnLevel = 24 },
				new LearnableMove { MoveId = "static_field", LearnLevel = 26 },
				new LearnableMove { MoveId = "power_sap", LearnLevel = 28 },
				new LearnableMove { MoveId = "tidal_slam", LearnLevel = 30 },
				new LearnableMove { MoveId = "chain_lightning", LearnLevel = 32 },
				new LearnableMove { MoveId = "brace", LearnLevel = 34 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "stun_wave", LearnLevel = 10 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 12 },
				new LearnableMove { MoveId = "volt_charge", LearnLevel = 14 },
				new LearnableMove { MoveId = "charge_up", LearnLevel = 16 },
				new LearnableMove { MoveId = "spark_wave", LearnLevel = 18 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 20 },
				new LearnableMove { MoveId = "void_sphere", LearnLevel = 22 },
				new LearnableMove { MoveId = "bolt_rush", LearnLevel = 24 },
				new LearnableMove { MoveId = "static_field", LearnLevel = 26 },
				new LearnableMove { MoveId = "overcharge", LearnLevel = 28 },
				new LearnableMove { MoveId = "arc_bolt", LearnLevel = 30 },
				new LearnableMove { MoveId = "chain_lightning", LearnLevel = 34 },
				new LearnableMove { MoveId = "plasma_charge", LearnLevel = 36 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "charge_up", LearnLevel = 10 },
				new LearnableMove { MoveId = "volt_charge", LearnLevel = 12 },
				new LearnableMove { MoveId = "spark_wave", LearnLevel = 14 },
				new LearnableMove { MoveId = "static_field", LearnLevel = 16 },
				new LearnableMove { MoveId = "temper", LearnLevel = 18 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 19 },
				new LearnableMove { MoveId = "bolt_rush", LearnLevel = 20 },
				new LearnableMove { MoveId = "concentration", LearnLevel = 22 },
				new LearnableMove { MoveId = "shock_tackle", LearnLevel = 23 },
				new LearnableMove { MoveId = "power_sap", LearnLevel = 24 },
				new LearnableMove { MoveId = "arc_bolt", LearnLevel = 26 },
				new LearnableMove { MoveId = "circuit_burst", LearnLevel = 27 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 12 },
				new LearnableMove { MoveId = "tremor_pulse", LearnLevel = 14 },
				new LearnableMove { MoveId = "earthrend", LearnLevel = 16 },
				new LearnableMove { MoveId = "dust_storm", LearnLevel = 18 },
				new LearnableMove { MoveId = "terra_pulse", LearnLevel = 20 },
				new LearnableMove { MoveId = "bedrock_stance", LearnLevel = 21 },
				new LearnableMove { MoveId = "sandstone_strike", LearnLevel = 22 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 23 },
				new LearnableMove { MoveId = "stalagmite_jab", LearnLevel = 24 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 25 },
				new LearnableMove { MoveId = "ground_pulse", LearnLevel = 26 },
				new LearnableMove { MoveId = "quicksand", LearnLevel = 27 },
				new LearnableMove { MoveId = "geode_burst", LearnLevel = 28 },
				new LearnableMove { MoveId = "fault_breaker", LearnLevel = 30 }
			},
			BeastiaryNumber = 0,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "boulder_toss", LearnLevel = 1 },
				new LearnableMove { MoveId = "harden", LearnLevel = 1 },
				new LearnableMove { MoveId = "sandstone_strike", LearnLevel = 5 },
				new LearnableMove { MoveId = "dust_storm", LearnLevel = 8 },
				new LearnableMove { MoveId = "bedrock_stance", LearnLevel = 12 },
				new LearnableMove { MoveId = "tremor_pulse", LearnLevel = 16 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 18 },
				new LearnableMove { MoveId = "fault_breaker", LearnLevel = 20 },
				new LearnableMove { MoveId = "seismic_crash", LearnLevel = 22 },
				new LearnableMove { MoveId = "terra_pulse", LearnLevel = 22 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 24 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 26 },
				new LearnableMove { MoveId = "temper", LearnLevel = 28 },
				new LearnableMove { MoveId = "quicksand", LearnLevel = 30 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "temper", LearnLevel = 1 },
				new LearnableMove { MoveId = "tectonic_slam", LearnLevel = 5 },
				new LearnableMove { MoveId = "bedrock_stance", LearnLevel = 10 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 15 },
				new LearnableMove { MoveId = "fissure_wave", LearnLevel = 20 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 25 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 30 },
				new LearnableMove { MoveId = "landslide", LearnLevel = 34 },
				new LearnableMove { MoveId = "mineral_lance", LearnLevel = 37 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 40 },
				new LearnableMove { MoveId = "petrify", LearnLevel = 43 },
				new LearnableMove { MoveId = "continental_crush", LearnLevel = 46 },
				new LearnableMove { MoveId = "brace", LearnLevel = 48 },
				new LearnableMove { MoveId = "quake_stomp", LearnLevel = 50 },
				new LearnableMove { MoveId = "primeval_force", LearnLevel = 52 },
				new LearnableMove { MoveId = "world_crush", LearnLevel = 54 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 56 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "dust_storm", LearnLevel = 14 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 16 },
				new LearnableMove { MoveId = "sandstone_strike", LearnLevel = 18 },
				new LearnableMove { MoveId = "seismic_crash", LearnLevel = 20 },
				new LearnableMove { MoveId = "bedrock_stance", LearnLevel = 22 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 24 },
				new LearnableMove { MoveId = "tremor_pulse", LearnLevel = 25 },
				new LearnableMove { MoveId = "quicksand", LearnLevel = 26 },
				new LearnableMove { MoveId = "temper", LearnLevel = 28 },
				new LearnableMove { MoveId = "geode_burst", LearnLevel = 29 },
				new LearnableMove { MoveId = "ground_pulse", LearnLevel = 30 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "harden", LearnLevel = 1 },
				new LearnableMove { MoveId = "gravel_toss", LearnLevel = 3 },
				new LearnableMove { MoveId = "mud_hurl", LearnLevel = 5 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 7 },
				new LearnableMove { MoveId = "boulder_toss", LearnLevel = 8 },
				new LearnableMove { MoveId = "sand_blast", LearnLevel = 10 },
				new LearnableMove { MoveId = "sandstone_strike", LearnLevel = 12 },
				new LearnableMove { MoveId = "earthrend", LearnLevel = 14 },
				new LearnableMove { MoveId = "bedrock_stance", LearnLevel = 16 },
				new LearnableMove { MoveId = "terra_pulse", LearnLevel = 18 },
				new LearnableMove { MoveId = "dust_storm", LearnLevel = 19 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 20 },
				new LearnableMove { MoveId = "tremor_pulse", LearnLevel = 21 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 22 },
				new LearnableMove { MoveId = "quicksand", LearnLevel = 23 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 24 },
				new LearnableMove { MoveId = "ground_pulse", LearnLevel = 25 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "dust_storm", LearnLevel = 12 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 16 },
				new LearnableMove { MoveId = "terra_pulse", LearnLevel = 20 },
				new LearnableMove { MoveId = "brace", LearnLevel = 22 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 24 },
				new LearnableMove { MoveId = "fault_breaker", LearnLevel = 26 },
				new LearnableMove { MoveId = "quicksand", LearnLevel = 28 },
				new LearnableMove { MoveId = "seismic_crash", LearnLevel = 30 },
				new LearnableMove { MoveId = "tectonic_slam", LearnLevel = 32 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 34 },
				new LearnableMove { MoveId = "landslide", LearnLevel = 36 },
				new LearnableMove { MoveId = "jagged_spike", LearnLevel = 38 },
				new LearnableMove { MoveId = "petrify", LearnLevel = 40 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "boulder_toss", LearnLevel = 1 },
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "gravel_toss", LearnLevel = 4 },
				new LearnableMove { MoveId = "harden", LearnLevel = 6 },
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 8 },
				new LearnableMove { MoveId = "sand_blast", LearnLevel = 10 },
				new LearnableMove { MoveId = "geode_burst", LearnLevel = 12 },
				new LearnableMove { MoveId = "earthrend", LearnLevel = 14 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 16 },
				new LearnableMove { MoveId = "tremor_pulse", LearnLevel = 18 },
				new LearnableMove { MoveId = "mineral_lance", LearnLevel = 20 },
				new LearnableMove { MoveId = "seismic_crash", LearnLevel = 22 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 24 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 26 },
				new LearnableMove { MoveId = "fissure_wave", LearnLevel = 28 },
				new LearnableMove { MoveId = "gleaming_ray", LearnLevel = 30 },
				new LearnableMove { MoveId = "crystallize", LearnLevel = 32 },
				new LearnableMove { MoveId = "concentration", LearnLevel = 34 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "bedrock_stance", LearnLevel = 18 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 20 },
				new LearnableMove { MoveId = "temper", LearnLevel = 22 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 24 },
				new LearnableMove { MoveId = "quicksand", LearnLevel = 26 },
				new LearnableMove { MoveId = "tremor_pulse", LearnLevel = 28 },
				new LearnableMove { MoveId = "sandstone_strike", LearnLevel = 29 },
				new LearnableMove { MoveId = "ground_pulse", LearnLevel = 30 },
				new LearnableMove { MoveId = "jagged_spike", LearnLevel = 32 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 10 },
				new LearnableMove { MoveId = "sandstone_strike", LearnLevel = 12 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 14 },
				new LearnableMove { MoveId = "earthrend", LearnLevel = 16 },
				new LearnableMove { MoveId = "tremor_pulse", LearnLevel = 18 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 20 },
				new LearnableMove { MoveId = "fault_breaker", LearnLevel = 22 },
				new LearnableMove { MoveId = "seismic_crash", LearnLevel = 24 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 26 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 28 },
				new LearnableMove { MoveId = "tectonic_slam", LearnLevel = 30 },
				new LearnableMove { MoveId = "brace", LearnLevel = 32 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "cold_shoulder", LearnLevel = 3 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "chill_wind", LearnLevel = 7 },
				new LearnableMove { MoveId = "frost_ward", LearnLevel = 9 },
				new LearnableMove { MoveId = "harden", LearnLevel = 10 },
				new LearnableMove { MoveId = "winter_veil", LearnLevel = 12 },
				new LearnableMove { MoveId = "ice_fang", LearnLevel = 14 },
				new LearnableMove { MoveId = "numb_chill", LearnLevel = 16 },
				new LearnableMove { MoveId = "permafrost_ray", LearnLevel = 18 },
				new LearnableMove { MoveId = "frozen_claw", LearnLevel = 19 },
				new LearnableMove { MoveId = "frozen_ground", LearnLevel = 20 },
				new LearnableMove { MoveId = "rime_coat", LearnLevel = 21 },
				new LearnableMove { MoveId = "sleet_barrage", LearnLevel = 22 },
				new LearnableMove { MoveId = "hail_strike", LearnLevel = 23 },
				new LearnableMove { MoveId = "icicle_lance", LearnLevel = 24 },
				new LearnableMove { MoveId = "crystallize", LearnLevel = 25 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "chill_wind", LearnLevel = 4 },
				new LearnableMove { MoveId = "frost_ward", LearnLevel = 8 },
				new LearnableMove { MoveId = "frozen_ground", LearnLevel = 12 },
				new LearnableMove { MoveId = "ice_fang", LearnLevel = 14 },
				new LearnableMove { MoveId = "harden", LearnLevel = 16 },
				new LearnableMove { MoveId = "sleet_barrage", LearnLevel = 18 },
				new LearnableMove { MoveId = "numb_chill", LearnLevel = 20 },
				new LearnableMove { MoveId = "icicle_lance", LearnLevel = 22 },
				new LearnableMove { MoveId = "temper", LearnLevel = 24 },
				new LearnableMove { MoveId = "cryo_beam", LearnLevel = 26 },
				new LearnableMove { MoveId = "glacier_slam", LearnLevel = 28 },
				new LearnableMove { MoveId = "ice_armor", LearnLevel = 30 },
				new LearnableMove { MoveId = "avalanche_wrath", LearnLevel = 32 },
				new LearnableMove { MoveId = "brace", LearnLevel = 34 },
				new LearnableMove { MoveId = "glacial_mirror", LearnLevel = 36 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "temper", LearnLevel = 1 },
				new LearnableMove { MoveId = "winter_veil", LearnLevel = 1 },
				new LearnableMove { MoveId = "cryo_beam", LearnLevel = 5 },
				new LearnableMove { MoveId = "ice_armor", LearnLevel = 10 },
				new LearnableMove { MoveId = "glacial_mirror", LearnLevel = 15 },
				new LearnableMove { MoveId = "deep_freeze", LearnLevel = 20 },
				new LearnableMove { MoveId = "blizzard_wrath", LearnLevel = 25 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "frostbite_nova", LearnLevel = 30 },
				new LearnableMove { MoveId = "frost_crush", LearnLevel = 32 },
				new LearnableMove { MoveId = "subzero_pulse", LearnLevel = 36 },
				new LearnableMove { MoveId = "glacier_slam", LearnLevel = 40 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 43 },
				new LearnableMove { MoveId = "polar_charge", LearnLevel = 45 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 48 },
				new LearnableMove { MoveId = "shatter_punch", LearnLevel = 50 },
				new LearnableMove { MoveId = "absolute_zero", LearnLevel = 53 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 56 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "frost_breath", LearnLevel = 1 },
				new LearnableMove { MoveId = "rend", LearnLevel = 1 },
				new LearnableMove { MoveId = "cold_shoulder", LearnLevel = 4 },
				new LearnableMove { MoveId = "numb_chill", LearnLevel = 6 },
				new LearnableMove { MoveId = "winter_veil", LearnLevel = 8 },
				new LearnableMove { MoveId = "harden", LearnLevel = 10 },
				new LearnableMove { MoveId = "chill_wind", LearnLevel = 12 },
				new LearnableMove { MoveId = "ice_fang", LearnLevel = 14 },
				new LearnableMove { MoveId = "permafrost_ray", LearnLevel = 16 },
				new LearnableMove { MoveId = "frost_ward", LearnLevel = 18 },
				new LearnableMove { MoveId = "icicle_lance", LearnLevel = 20 },
				new LearnableMove { MoveId = "frozen_ground", LearnLevel = 22 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 24 },
				new LearnableMove { MoveId = "sleet_barrage", LearnLevel = 26 },
				new LearnableMove { MoveId = "cryo_beam", LearnLevel = 28 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 30 },
				new LearnableMove { MoveId = "glacier_slam", LearnLevel = 32 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "cold_shoulder", LearnLevel = 3 },
				new LearnableMove { MoveId = "chill_wind", LearnLevel = 5 },
				new LearnableMove { MoveId = "harden", LearnLevel = 7 },
				new LearnableMove { MoveId = "winter_veil", LearnLevel = 8 },
				new LearnableMove { MoveId = "frost_ward", LearnLevel = 10 },
				new LearnableMove { MoveId = "numb_chill", LearnLevel = 12 },
				new LearnableMove { MoveId = "ice_fang", LearnLevel = 14 },
				new LearnableMove { MoveId = "permafrost_ray", LearnLevel = 16 },
				new LearnableMove { MoveId = "frozen_ground", LearnLevel = 18 },
				new LearnableMove { MoveId = "sleet_barrage", LearnLevel = 20 },
				new LearnableMove { MoveId = "updraft", LearnLevel = 22 },
				new LearnableMove { MoveId = "rime_coat", LearnLevel = 23 },
				new LearnableMove { MoveId = "icicle_lance", LearnLevel = 24 },
				new LearnableMove { MoveId = "hail_strike", LearnLevel = 25 },
				new LearnableMove { MoveId = "cryo_beam", LearnLevel = 26 },
				new LearnableMove { MoveId = "crystallize", LearnLevel = 27 },
				new LearnableMove { MoveId = "glacier_slam", LearnLevel = 28 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "frost_ward", LearnLevel = 8 },
				new LearnableMove { MoveId = "frozen_ground", LearnLevel = 12 },
				new LearnableMove { MoveId = "sleet_barrage", LearnLevel = 16 },
				new LearnableMove { MoveId = "numb_chill", LearnLevel = 18 },
				new LearnableMove { MoveId = "icicle_lance", LearnLevel = 20 },
				new LearnableMove { MoveId = "harden", LearnLevel = 22 },
				new LearnableMove { MoveId = "frost_crush", LearnLevel = 24 },
				new LearnableMove { MoveId = "cryo_beam", LearnLevel = 26 },
				new LearnableMove { MoveId = "glacier_slam", LearnLevel = 28 },
				new LearnableMove { MoveId = "avalanche_wrath", LearnLevel = 30 },
				new LearnableMove { MoveId = "ice_armor", LearnLevel = 32 },
				new LearnableMove { MoveId = "blizzard_wrath", LearnLevel = 36 },
				new LearnableMove { MoveId = "brace", LearnLevel = 38 },
				new LearnableMove { MoveId = "glacial_mirror", LearnLevel = 40 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "rend", LearnLevel = 1 },
				new LearnableMove { MoveId = "cold_shoulder", LearnLevel = 3 },
				new LearnableMove { MoveId = "ice_fang", LearnLevel = 5 },
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 6 },
				new LearnableMove { MoveId = "frozen_claw", LearnLevel = 8 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 10 },
				new LearnableMove { MoveId = "numb_chill", LearnLevel = 12 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 14 },
				new LearnableMove { MoveId = "frost_ward", LearnLevel = 16 },
				new LearnableMove { MoveId = "hail_strike", LearnLevel = 18 },
				new LearnableMove { MoveId = "winter_grasp", LearnLevel = 20 },
				new LearnableMove { MoveId = "permafrost_ray", LearnLevel = 22 },
				new LearnableMove { MoveId = "shatter_punch", LearnLevel = 24 },
				new LearnableMove { MoveId = "rime_coat", LearnLevel = 26 },
				new LearnableMove { MoveId = "polar_charge", LearnLevel = 28 },
				new LearnableMove { MoveId = "frozen_ground", LearnLevel = 30 },
				new LearnableMove { MoveId = "avalanche_wrath", LearnLevel = 32 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "cold_shoulder", LearnLevel = 3 },
				new LearnableMove { MoveId = "chill_wind", LearnLevel = 5 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 7 },
				new LearnableMove { MoveId = "winter_veil", LearnLevel = 8 },
				new LearnableMove { MoveId = "numb_chill", LearnLevel = 10 },
				new LearnableMove { MoveId = "frost_ward", LearnLevel = 12 },
				...
				new LearnableMove { MoveId = "icicle_lance", LearnLevel = 24 },
				new LearnableMove { MoveId = "cryo_beam", LearnLevel = 26 },
				new LearnableMove { MoveId = "crystallize", LearnLevel = 28 },
				new LearnableMove { MoveId = "void_sphere", LearnLevel = 30 },
				new LearnableMove { MoveId = "glacial_mirror", LearnLevel = 32 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "harden", LearnLevel = 7 },
				new LearnableMove { MoveId = "winter_veil", LearnLevel = 8 },
				new LearnableMove { MoveId = "frozen_claw", LearnLevel = 10 },
				new LearnableMove { MoveId = "numb_chill", LearnLevel = 12 },
				new LearnableMove { MoveId = "frost_ward", LearnLevel = 14 },
				new LearnableMove { MoveId = "permafrost_ray", LearnLevel = 16 },
				new LearnableMove { MoveId = "hail_strike", LearnLevel = 18 },
				new LearnableMove { MoveId = "winter_grasp", LearnLevel = 20 },
				new LearnableMove { MoveId = "rime_coat", LearnLevel = 22 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 24 },
				new LearnableMove { MoveId = "shatter_punch", LearnLevel = 26 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "polar_charge", LearnLevel = 30 },
				new LearnableMove { MoveId = "glacier_slam", LearnLevel = 32 },
				new LearnableMove { MoveId = "avalanche_wrath", LearnLevel = 34 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "frost_ward", LearnLevel = 10 },
				new LearnableMove { MoveId = "numb_chill", LearnLevel = 12 },
				new LearnableMove { MoveId = "ice_armor", LearnLevel = 14 },
				new LearnableMove { MoveId = "chill_wind", LearnLevel = 16 },
				new LearnableMove { MoveId = "permafrost_ray", LearnLevel = 18 },
				new LearnableMove { MoveId = "frozen_ground", LearnLevel = 20 },
				new LearnableMove { MoveId = "rime_coat", LearnLevel = 22 },
				new LearnableMove { MoveId = "glacier_slam", LearnLevel = 24 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 26 },
				new LearnableMove { MoveId = "temper", LearnLevel = 28 },
				new LearnableMove { MoveId = "frost_crush", LearnLevel = 28 },
				new LearnableMove { MoveId = "glacial_mirror", LearnLevel = 32 },
				new LearnableMove { MoveId = "blizzard_wrath", LearnLevel = 35 },
				new LearnableMove { MoveId = "deep_freeze", LearnLevel = 38 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "seed_volley", LearnLevel = 12 },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 15 },
				new LearnableMove { MoveId = "tangling_roots", LearnLevel = 16 },
				new LearnableMove { MoveId = "root_bind", LearnLevel = 18 },
				new LearnableMove { MoveId = "brace", LearnLevel = 19 },
				new LearnableMove { MoveId = "pollen_burst", LearnLevel = 20 },
				new LearnableMove { MoveId = "spore_cloud", LearnLevel = 21 },
				new LearnableMove { MoveId = "vitality_burst", LearnLevel = 22 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 23 },
				new LearnableMove { MoveId = "wild_thorn", LearnLevel = 24 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 25 },
				new LearnableMove { MoveId = "bloom_burst", LearnLevel = 26 },
				new LearnableMove { MoveId = "growth_surge", LearnLevel = 28 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "root_bind", LearnLevel = 20 },
				...
				new LearnableMove { MoveId = "tangling_roots", LearnLevel = 32 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 33 },
				new LearnableMove { MoveId = "timber_slam", LearnLevel = 34 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 36 },
				new LearnableMove { MoveId = "canopy_crash", LearnLevel = 37 },
				new LearnableMove { MoveId = "toxic_pollen", LearnLevel = 38 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 40 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "harden", LearnLevel = 10 },
				new LearnableMove { MoveId = "vine_crush", LearnLevel = 18 },
				new LearnableMove { MoveId = "verdant_edge", LearnLevel = 24 },
				new LearnableMove { MoveId = "canopy_crash", LearnLevel = 30 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 34 },
				new LearnableMove { MoveId = "brace", LearnLevel = 36 },
				new LearnableMove { MoveId = "overgrowth_slam", LearnLevel = 38 },
				new LearnableMove { MoveId = "toxic_pollen", LearnLevel = 40 },
				new LearnableMove { MoveId = "timber_slam", LearnLevel = 42 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 44 },
				new LearnableMove { MoveId = "jungle_wrath", LearnLevel = 46 },
				new LearnableMove { MoveId = "primal_roar", LearnLevel = 48 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "intimidate", LearnLevel = 1 },
				new LearnableMove { MoveId = "thorn_lash", LearnLevel = 5 },
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 8 },
				new LearnableMove { MoveId = "spore_cloud", LearnLevel = 10 },
				new LearnableMove { MoveId = "harden", LearnLevel = 12 },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 14 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 16 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 18 },
				new LearnableMove { MoveId = "pollen_burst", LearnLevel = 19 },
				new LearnableMove { MoveId = "parasite_spore", LearnLevel = 20 },
				new LearnableMove { MoveId = "brace", LearnLevel = 21 },
				new LearnableMove { MoveId = "toxic_pollen", LearnLevel = 22 },
				new LearnableMove { MoveId = "vitality_burst", LearnLevel = 23 },
				new LearnableMove { MoveId = "root_bind", LearnLevel = 24 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 25 },
				new LearnableMove { MoveId = "bloom_burst", LearnLevel = 26 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 28 },
				new LearnableMove { MoveId = "growth_surge", LearnLevel = 30 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 32 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BaseCatchRate = 0.6f,
			BaseExpYield = 70,
			SignatureDropName = "Pollen Burst",
			SignatureDropDescription = "A fluffy clump of golden pollen, still faintly warm from a Pollenpuff's sneeze.",
			Personality = BeastPersonality.Timid,
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "spore_cloud", LearnLevel = 9 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 10 },
				new LearnableMove { MoveId = "floral_guard", LearnLevel = 12 },
				new LearnableMove { MoveId = "seed_volley", LearnLevel = 14 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 16 },
				new LearnableMove { MoveId = "pollen_burst", LearnLevel = 18 },
				new LearnableMove { MoveId = "toxic_pollen", LearnLevel = 19 },
				new LearnableMove { MoveId = "brace", LearnLevel = 20 },
				new LearnableMove { MoveId = "bloom_burst", LearnLevel = 21 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 22 },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 23 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 24 },
				new LearnableMove { MoveId = "vitality_burst", LearnLevel = 25 },
				new LearnableMove { MoveId = "growth_surge", LearnLevel = 26 },
				new LearnableMove { MoveId = "blossom_frenzy", LearnLevel = 28 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 10 },
				new LearnableMove { MoveId = "harden", LearnLevel = 14 },
				new LearnableMove { MoveId = "thorn_barrage", LearnLevel = 18 },
				new LearnableMove { MoveId = "vine_crush", LearnLevel = 22 },
				new LearnableMove { MoveId = "toxic_pollen", LearnLevel = 24 },
				new LearnableMove { MoveId = "verdant_edge", LearnLevel = 28 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 30 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 32 },
				new LearnableMove { MoveId = "canopy_crash", LearnLevel = 34 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "strike", LearnLevel = 4 },
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 6 },
				new LearnableMove { MoveId = "harden", LearnLevel = 8 },
				new LearnableMove { MoveId = "bramble_strike", LearnLevel = 10 },
				new LearnableMove { MoveId = "vine_crush", LearnLevel = 12 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 14 },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 16 },
				new LearnableMove { MoveId = "tangling_roots", LearnLevel = 17 },
				new LearnableMove { MoveId = "wild_thorn", LearnLevel = 18 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 19 },
				new LearnableMove { MoveId = "brace", LearnLevel = 20 },
				new LearnableMove { MoveId = "seed_volley", LearnLevel = 21 },
				new LearnableMove { MoveId = "verdant_edge", LearnLevel = 22 },
				new LearnableMove { MoveId = "thorn_barrage", LearnLevel = 23 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 24 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 25 },
				new LearnableMove { MoveId = "feral_charge", LearnLevel = 26 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 28 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 1 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "thorn_lash", LearnLevel = 7 },
				new LearnableMove { MoveId = "bloom_burst", LearnLevel = 10 },
				new LearnableMove { MoveId = "spore_cloud", LearnLevel = 13 },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 16 },
				new LearnableMove { MoveId = "harden", LearnLevel = 18 },
				new LearnableMove { MoveId = "parasite_spore", LearnLevel = 20 },
				new LearnableMove { MoveId = "vitality_burst", LearnLevel = 22 },
				new LearnableMove { MoveId = "toxic_pollen", LearnLevel = 24 },
				new LearnableMove { MoveId = "brace", LearnLevel = 26 },
				new LearnableMove { MoveId = "floral_guard", LearnLevel = 28 },
				new LearnableMove { MoveId = "thorn_barrage", LearnLevel = 30 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 32 },
				new LearnableMove { MoveId = "blossom_frenzy", LearnLevel = 34 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 35 },
				new LearnableMove { MoveId = "verdant_edge", LearnLevel = 36 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 38 },
				new LearnableMove { MoveId = "growth_surge", LearnLevel = 40 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "cinders_curse", LearnLevel = 1 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "spore_cloud", LearnLevel = 7 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 10 },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 12 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 14 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 16 },
				new LearnableMove { MoveId = "vitality_burst", LearnLevel = 18 },
				new LearnableMove { MoveId = "pollen_burst", LearnLevel = 19 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 20 },
				new LearnableMove { MoveId = "toxic_pollen", LearnLevel = 21 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 22 },
				new LearnableMove { MoveId = "brace", LearnLevel = 23 },
				new LearnableMove { MoveId = "divine_grace", LearnLevel = 24 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 26 },
				new LearnableMove { MoveId = "bloom_burst", LearnLevel = 27 },
				new LearnableMove { MoveId = "growth_surge", LearnLevel = 28 },
				new LearnableMove { MoveId = "photon_bloom", LearnLevel = 30 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 32 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 12 },
				new LearnableMove { MoveId = "blossom_frenzy", LearnLevel = 15 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 18 },
				new LearnableMove { MoveId = "vine_crush", LearnLevel = 20 },
				new LearnableMove { MoveId = "thorn_barrage", LearnLevel = 22 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 25 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "canopy_crash", LearnLevel = 30 },
				new LearnableMove { MoveId = "growth_surge", LearnLevel = 32 },
				new LearnableMove { MoveId = "divine_grace", LearnLevel = 35 },
				new LearnableMove { MoveId = "overgrowth_slam", LearnLevel = 38 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 40 },
				new LearnableMove { MoveId = "photon_bloom", LearnLevel = 42 },
				new LearnableMove { MoveId = "jungle_wrath", LearnLevel = 45 },
				new LearnableMove { MoveId = "evergreen_pulse", LearnLevel = 48 },
				new LearnableMove { MoveId = "primal_roar", LearnLevel = 50 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "rivet_strike", LearnLevel = 16 },
				new LearnableMove { MoveId = "cog_crush", LearnLevel = 18 },
				new LearnableMove { MoveId = "brace", LearnLevel = 19 },
				new LearnableMove { MoveId = "rapid_shear", LearnLevel = 20 },
				new LearnableMove { MoveId = "iron_rush", LearnLevel = 21 },
				new LearnableMove { MoveId = "magnet_rise", LearnLevel = 22 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 23 },
				new LearnableMove { MoveId = "slag_shot", LearnLevel = 24 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 25 },
				new LearnableMove { MoveId = "iron_rebound", LearnLevel = 26 },
				new LearnableMove { MoveId = "rust_curse", LearnLevel = 27 },
				new LearnableMove { MoveId = "spin_crash", LearnLevel = 28 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 29 },
				new LearnableMove { MoveId = "scrap_barrage", LearnLevel = 30 },
				new LearnableMove { MoveId = "anvil_drop", LearnLevel = 32 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "temper", LearnLevel = 1 },
				new LearnableMove { MoveId = "harden", LearnLevel = 1 },
				new LearnableMove { MoveId = "steel_rake", LearnLevel = 5 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 10 },
				new LearnableMove { MoveId = "rivet_strike", LearnLevel = 14 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 18 },
				new LearnableMove { MoveId = "magnet_rise", LearnLevel = 22 },
				new LearnableMove { MoveId = "titan_drop", LearnLevel = 26 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "rapid_shear", LearnLevel = 30 },
				new LearnableMove { MoveId = "iron_rebound", LearnLevel = 34 },
				new LearnableMove { MoveId = "rust_curse", LearnLevel = 36 },
				new LearnableMove { MoveId = "anvil_drop", LearnLevel = 38 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 39 },
				new LearnableMove { MoveId = "scrap_barrage", LearnLevel = 40 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 41 },
				new LearnableMove { MoveId = "foundry_crash", LearnLevel = 42 },
				new LearnableMove { MoveId = "alloy_smash", LearnLevel = 44 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "steel_rake", LearnLevel = 5 },
				new LearnableMove { MoveId = "harden", LearnLevel = 10 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 16 },
				new LearnableMove { MoveId = "anvil_drop", LearnLevel = 22 },
				new LearnableMove { MoveId = "brace", LearnLevel = 26 },
				new LearnableMove { MoveId = "rapid_shear", LearnLevel = 30 },
				new LearnableMove { MoveId = "meltdown_beam", LearnLevel = 36 },
				new LearnableMove { MoveId = "foundry_crash", LearnLevel = 38 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 40 },
				new LearnableMove { MoveId = "steel_rend", LearnLevel = 42 },
				new LearnableMove { MoveId = "gleaming_ray", LearnLevel = 45 },
				new LearnableMove { MoveId = "forge_blast", LearnLevel = 48 },
				new LearnableMove { MoveId = "primal_roar", LearnLevel = 50 },
				new LearnableMove { MoveId = "updraft", LearnLevel = 52 },
				new LearnableMove { MoveId = "alloy_smash", LearnLevel = 54 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 55 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "rust_curse", LearnLevel = 8 },
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 12 },
				new LearnableMove { MoveId = "slag_shot", LearnLevel = 14 },
				new LearnableMove { MoveId = "temper", LearnLevel = 16 },
				new LearnableMove { MoveId = "corrosive_breath", LearnLevel = 18 },
				new LearnableMove { MoveId = "oxidize_ray", LearnLevel = 20 },
				new LearnableMove { MoveId = "harden", LearnLevel = 22 },
				new LearnableMove { MoveId = "iron_rush", LearnLevel = 26 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "gleaming_ray", LearnLevel = 30 },
				new LearnableMove { MoveId = "mercury_surge", LearnLevel = 32 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 34 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 36 },
				new LearnableMove { MoveId = "meltdown_beam", LearnLevel = 38 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 39 },
				new LearnableMove { MoveId = "scrap_barrage", LearnLevel = 40 },
				new LearnableMove { MoveId = "alloy_smash", LearnLevel = 42 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "void_sphere", LearnLevel = 1 },
				new LearnableMove { MoveId = "steel_rake", LearnLevel = 5 },
				new LearnableMove { MoveId = "rust_curse", LearnLevel = 10 },
				new LearnableMove { MoveId = "temper", LearnLevel = 16 },
				new LearnableMove { MoveId = "corrosive_breath", LearnLevel = 22 },
				new LearnableMove { MoveId = "oxidize_ray", LearnLevel = 26 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "meltdown_beam", LearnLevel = 32 },
				new LearnableMove { MoveId = "umbral_claw", LearnLevel = 36 },
				new LearnableMove { MoveId = "mercury_surge", LearnLevel = 38 },
				new LearnableMove { MoveId = "forge_blast", LearnLevel = 40 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 42 },
				new LearnableMove { MoveId = "steel_rend", LearnLevel = 44 },
				new LearnableMove { MoveId = "foundry_crash", LearnLevel = 46 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 48 },
				new LearnableMove { MoveId = "starfall_strike", LearnLevel = 50 },
				new LearnableMove { MoveId = "alloy_smash", LearnLevel = 52 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "rivet_strike", LearnLevel = 16 },
				new LearnableMove { MoveId = "brace", LearnLevel = 18 },
				new LearnableMove { MoveId = "cog_crush", LearnLevel = 19 },
				new LearnableMove { MoveId = "iron_rush", LearnLevel = 22 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 23 },
				new LearnableMove { MoveId = "temper", LearnLevel = 24 },
				new LearnableMove { MoveId = "slag_shot", LearnLevel = 25 },
				new LearnableMove { MoveId = "rapid_shear", LearnLevel = 26 },
				new LearnableMove { MoveId = "rust_curse", LearnLevel = 27 },
				new LearnableMove { MoveId = "spin_crash", LearnLevel = 28 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 29 },
				new LearnableMove { MoveId = "scrap_barrage", LearnLevel = 30 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 31 },
				new LearnableMove { MoveId = "iron_rebound", LearnLevel = 32 },
				new LearnableMove { MoveId = "anvil_drop", LearnLevel = 34 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "harden", LearnLevel = 1 },
				new LearnableMove { MoveId = "iron_rush", LearnLevel = 5 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 10 },
				new LearnableMove { MoveId = "temper", LearnLevel = 14 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 18 },
				new LearnableMove { MoveId = "brace", LearnLevel = 22 },
				new LearnableMove { MoveId = "rivet_strike", LearnLevel = 26 },
				new LearnableMove { MoveId = "anvil_drop", LearnLevel = 30 },
				new LearnableMove { MoveId = "spin_crash", LearnLevel = 34 },
				new LearnableMove { MoveId = "rust_curse", LearnLevel = 36 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 37 },
				new LearnableMove { MoveId = "scrap_barrage", LearnLevel = 38 },
				new LearnableMove { MoveId = "foundry_crash", LearnLevel = 40 },
				new LearnableMove { MoveId = "jagged_spike", LearnLevel = 42 },
				new LearnableMove { MoveId = "steel_rend", LearnLevel = 44 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 45 },
				new LearnableMove { MoveId = "alloy_smash", LearnLevel = 46 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 11 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 13 },
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 16 },
				new LearnableMove { MoveId = "rivet_strike", LearnLevel = 18 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 19 },
				new LearnableMove { MoveId = "chrome_wing", LearnLevel = 23 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 24 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 25 },
				new LearnableMove { MoveId = "temper", LearnLevel = 26 },
				new LearnableMove { MoveId = "iron_rush", LearnLevel = 27 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "cog_crush", LearnLevel = 29 },
				new LearnableMove { MoveId = "slag_shot", LearnLevel = 30 },
				new LearnableMove { MoveId = "scrap_barrage", LearnLevel = 31 },
				new LearnableMove { MoveId = "spin_crash", LearnLevel = 32 },
				new LearnableMove { MoveId = "steel_rend", LearnLevel = 34 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 36 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "burnish", LearnLevel = 14 },
				...
				new LearnableMove { MoveId = "rust_curse", LearnLevel = 30 },
				new LearnableMove { MoveId = "iron_rebound", LearnLevel = 32 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 34 },
				new LearnableMove { MoveId = "magnet_rise", LearnLevel = 35 },
				new LearnableMove { MoveId = "slag_shot", LearnLevel = 36 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 37 },
				new LearnableMove { MoveId = "oxidize_ray", LearnLevel = 38 },
				new LearnableMove { MoveId = "alloy_smash", LearnLevel = 40 },
				new LearnableMove { MoveId = "forge_blast", LearnLevel = 42 },
				new LearnableMove { MoveId = "concentration", LearnLevel = 44 },
				new LearnableMove { MoveId = "foundry_crash", LearnLevel = 46 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "root_bind", LearnLevel = 1 },
				new LearnableMove { MoveId = "harden", LearnLevel = 1 },
				new LearnableMove { MoveId = "strike", LearnLevel = 5 },
				new LearnableMove { MoveId = "steel_rake", LearnLevel = 8 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 10 },
				new LearnableMove { MoveId = "temper", LearnLevel = 15 },
				new LearnableMove { MoveId = "rivet_strike", LearnLevel = 17 },
				new LearnableMove { MoveId = "brace", LearnLevel = 19 },
				new LearnableMove { MoveId = "spin_crash", LearnLevel = 22 },
				new LearnableMove { MoveId = "iron_rush", LearnLevel = 24 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 25 },
				new LearnableMove { MoveId = "rust_curse", LearnLevel = 26 },
				new LearnableMove { MoveId = "magnet_rise", LearnLevel = 27 },
				new LearnableMove { MoveId = "slag_shot", LearnLevel = 28 },
				new LearnableMove { MoveId = "iron_rebound", LearnLevel = 29 },
				new LearnableMove { MoveId = "scrap_barrage", LearnLevel = 30 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 31 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 32 },
				new LearnableMove { MoveId = "cog_crush", LearnLevel = 34 },
				new LearnableMove { MoveId = "anvil_drop", LearnLevel = 36 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "strike", LearnLevel = 5 },
				new LearnableMove { MoveId = "harden", LearnLevel = 8 },
				new LearnableMove { MoveId = "temper", LearnLevel = 12 },
				new LearnableMove { MoveId = "gleaming_ray", LearnLevel = 18 },
				new LearnableMove { MoveId = "iron_rush", LearnLevel = 22 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 24 },
				...
				new LearnableMove { MoveId = "chrome_wing", LearnLevel = 40 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 42 },
				new LearnableMove { MoveId = "forge_blast", LearnLevel = 44 },
				new LearnableMove { MoveId = "steel_rend", LearnLevel = 46 },
				new LearnableMove { MoveId = "foundry_crash", LearnLevel = 48 },
				new LearnableMove { MoveId = "alloy_smash", LearnLevel = 50 },
				new LearnableMove { MoveId = "primal_roar", LearnLevel = 52 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 55 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "bolster", LearnLevel = 10 },
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 12 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 14 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 18 },
				new LearnableMove { MoveId = "brace", LearnLevel = 19 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 20 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 21 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 22 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 23 },
				new LearnableMove { MoveId = "lunar_radiance", LearnLevel = 24 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 25 },
				new LearnableMove { MoveId = "divine_grace", LearnLevel = 26 },
				new LearnableMove { MoveId = "spectral_rush", LearnLevel = 27 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 28 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "bolster", LearnLevel = 10 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 14 },
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 18 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 22 },
				new LearnableMove { MoveId = "spectral_rush", LearnLevel = 26 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 30 },
				new LearnableMove { MoveId = "divine_grace", LearnLevel = 34 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 36 },
				new LearnableMove { MoveId = "radiant_burst", LearnLevel = 38 },
				new LearnableMove { MoveId = "ethereal_rush", LearnLevel = 40 },
				new LearnableMove { MoveId = "aurora_wave", LearnLevel = 42 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 44 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 10 },
				new LearnableMove { MoveId = "bolster", LearnLevel = 16 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 22 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 28 },
				new LearnableMove { MoveId = "spectral_rush", LearnLevel = 34 },
				new LearnableMove { MoveId = "brace", LearnLevel = 36 },
				new LearnableMove { MoveId = "radiant_burst", LearnLevel = 38 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 40 },
				new LearnableMove { MoveId = "aurora_wave", LearnLevel = 42 },
				new LearnableMove { MoveId = "celestial_flare", LearnLevel = 44 },
				new LearnableMove { MoveId = "judgment_blow", LearnLevel = 46 },
				new LearnableMove { MoveId = "avalanche_wrath", LearnLevel = 48 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 50 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "aether_pulse", LearnLevel = 1 },
				new LearnableMove { MoveId = "spirit_breeze", LearnLevel = 5 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 8 },
				new LearnableMove { MoveId = "bolster", LearnLevel = 12 },
				new LearnableMove { MoveId = "spirit_rend", LearnLevel = 16 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 22 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 24 },
				new LearnableMove { MoveId = "dream_feast", LearnLevel = 28 },
				new LearnableMove { MoveId = "brace", LearnLevel = 30 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 32 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 34 },
				new LearnableMove { MoveId = "lunar_radiance", LearnLevel = 36 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 37 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 38 },
				new LearnableMove { MoveId = "radiant_burst", LearnLevel = 40 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 41 },
				new LearnableMove { MoveId = "ethereal_rush", LearnLevel = 42 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "strike", LearnLevel = 5 },
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 8 },
				new LearnableMove { MoveId = "divine_grace", LearnLevel = 10 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 12 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 14 },
				new LearnableMove { MoveId = "aether_pulse", LearnLevel = 16 },
				new LearnableMove { MoveId = "brace", LearnLevel = 18 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 20 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 22 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 23 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 24 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 25 },
				new LearnableMove { MoveId = "lunar_radiance", LearnLevel = 26 },
				new LearnableMove { MoveId = "spectral_rush", LearnLevel = 27 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 28 },
				new LearnableMove { MoveId = "ethereal_rush", LearnLevel = 29 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "bolster", LearnLevel = 1 },
				new LearnableMove { MoveId = "spirit_breeze", LearnLevel = 5 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 10 },
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 14 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 18 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 22 },
				new LearnableMove { MoveId = "brace", LearnLevel = 24 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 28 },
				new LearnableMove { MoveId = "spectral_rush", LearnLevel = 30 },
				new LearnableMove { MoveId = "radiant_burst", LearnLevel = 32 },
				new LearnableMove { MoveId = "ethereal_rush", LearnLevel = 34 },
				new LearnableMove { MoveId = "spirit_rend", LearnLevel = 38 },
				new LearnableMove { MoveId = "aurora_wave", LearnLevel = 40 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 41 },
				new LearnableMove { MoveId = "celestial_flare", LearnLevel = 42 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 24 },
				new LearnableMove { MoveId = "spirit_rend", LearnLevel = 28 },
				new LearnableMove { MoveId = "brace", LearnLevel = 30 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 32 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 34 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 36 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 37 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 38 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 39 },
				new LearnableMove { MoveId = "radiant_burst", LearnLevel = 40 },
				new LearnableMove { MoveId = "ethereal_rush", LearnLevel = 42 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 43 },
				new LearnableMove { MoveId = "celestial_flare", LearnLevel = 44 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "spirit_breeze", LearnLevel = 5 },
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 8 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 10 },
				new LearnableMove { MoveId = "dream_feast", LearnLevel = 14 },
				new LearnableMove { MoveId = "aether_pulse", LearnLevel = 16 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 18 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 20 },
				new LearnableMove { MoveId = "bolster", LearnLevel = 22 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 24 },
				new LearnableMove { MoveId = "brace", LearnLevel = 25 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 26 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 27 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 28 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 29 },
				new LearnableMove { MoveId = "lunar_radiance", LearnLevel = 30 },
				new LearnableMove { MoveId = "spectral_rush", LearnLevel = 31 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 32 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 8 },
				new LearnableMove { MoveId = "bolster", LearnLevel = 12 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 16 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 22 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 24 },
				...
				new LearnableMove { MoveId = "brace", LearnLevel = 32 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 34 },
				new LearnableMove { MoveId = "spectral_rush", LearnLevel = 36 },
				new LearnableMove { MoveId = "radiant_burst", LearnLevel = 38 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 40 },
				new LearnableMove { MoveId = "ethereal_rush", LearnLevel = 42 },
				new LearnableMove { MoveId = "aurora_wave", LearnLevel = 44 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 45 },
				new LearnableMove { MoveId = "celestial_flare", LearnLevel = 46 },
				new LearnableMove { MoveId = "judgment_blow", LearnLevel = 48 },
			},
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 8 },
				new LearnableMove { MoveId = "bolster", LearnLevel = 12 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 16 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 18 },
				new LearnableMove { MoveId = "lunar_radiance", LearnLevel = 22 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 26 },
				new LearnableMove { MoveId = "spectral_rush", LearnLevel = 30 },
				new LearnableMove { MoveId = "spirit_rend", LearnLevel = 32 },
				new LearnableMove { MoveId = "brace", LearnLevel = 34 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 36 },
				new LearnableMove { MoveId = "radiant_burst", LearnLevel = 38 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 40 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "gloom_strike", LearnLevel = 10 },
				new LearnableMove { MoveId = "dusk_bolt", LearnLevel = 14 },
				new LearnableMove { MoveId = "sly_strike", LearnLevel = 16 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 18 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 20 },
				new LearnableMove { MoveId = "brace", LearnLevel = 21 },
				new LearnableMove { MoveId = "shadow_lunge", LearnLevel = 22 },
				new LearnableMove { MoveId = "dread_gaze", LearnLevel = 23 },
				new LearnableMove { MoveId = "murk_bolt", LearnLevel = 24 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 25 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 26 },
				new LearnableMove { MoveId = "void_sphere", LearnLevel = 27 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 28 },
				new LearnableMove { MoveId = "shadow_cloak", LearnLevel = 29 },
				new LearnableMove { MoveId = "umbral_claw", LearnLevel = 30 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 31 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "rend", LearnLevel = 1 },
				new LearnableMove { MoveId = "gloom_strike", LearnLevel = 5 },
				new LearnableMove { MoveId = "sly_strike", LearnLevel = 10 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 14 },
				new LearnableMove { MoveId = "shadow_lunge", LearnLevel = 18 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 22 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 24 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 28 },
				new LearnableMove { MoveId = "dread_gaze", LearnLevel = 30 },
				new LearnableMove { MoveId = "brace", LearnLevel = 32 },
				new LearnableMove { MoveId = "nightfall_rush", LearnLevel = 34 },
				new LearnableMove { MoveId = "umbral_claw", LearnLevel = 36 },
				new LearnableMove { MoveId = "shadow_cloak", LearnLevel = 38 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 39 },
				new LearnableMove { MoveId = "phantom_rend", LearnLevel = 40 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 41 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "dusk_bolt", LearnLevel = 1 },
				new LearnableMove { MoveId = "shade_step", LearnLevel = 5 },
				new LearnableMove { MoveId = "shadow_lunge", LearnLevel = 12 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 18 },
				new LearnableMove { MoveId = "phantom_rend", LearnLevel = 24 },
				new LearnableMove { MoveId = "nightfall_rush", LearnLevel = 28 },
				new LearnableMove { MoveId = "brace", LearnLevel = 32 },
				new LearnableMove { MoveId = "oblivion_claw", LearnLevel = 36 },
				new LearnableMove { MoveId = "shadow_cloak", LearnLevel = 38 },
				new LearnableMove { MoveId = "dread_gaze", LearnLevel = 40 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 42 },
				new LearnableMove { MoveId = "hex_blast", LearnLevel = 44 },
				new LearnableMove { MoveId = "shadow_wave", LearnLevel = 46 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 14 },
				new LearnableMove { MoveId = "sly_strike", LearnLevel = 16 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 18 },
				new LearnableMove { MoveId = "brace", LearnLevel = 19 },
				new LearnableMove { MoveId = "murk_bolt", LearnLevel = 20 },
				new LearnableMove { MoveId = "dread_gaze", LearnLevel = 21 },
				new LearnableMove { MoveId = "shadow_cloak", LearnLevel = 22 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 23 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 24 },
				new LearnableMove { MoveId = "void_sphere", LearnLevel = 26 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 27 },
				new LearnableMove { MoveId = "hex_blast", LearnLevel = 28 },
				new LearnableMove { MoveId = "shadow_wave", LearnLevel = 30 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 31 },
				new LearnableMove { MoveId = "nightfall_rush", LearnLevel = 32 },
			},
			BeastiaryNumber = 0
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "gloom_strike", LearnLevel = 16 },
				new LearnableMove { MoveId = "shadow_lunge", LearnLevel = 18 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 20 },
				new LearnableMove { MoveId = "brace", LearnLevel = 21 },
				new LearnableMove { MoveId = "dusk_bolt", LearnLevel = 22 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 23 },
				new LearnableMove { MoveId = "nightfall_rush", LearnLevel = 24 },
				new LearnableMove { MoveId = "shadow_cloak", LearnLevel = 25 },
				new LearnableMove { MoveId = "dread_gaze", LearnLevel = 26 },
				new LearnableMove { MoveId = "murk_bolt", LearnLevel = 27 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 28 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 29 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 30 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 5 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 10 },
				new LearnableMove { MoveId = "shadow_lunge", LearnLevel = 14 },
				new LearnableMove { MoveId = "nightfall_rush", LearnLevel = 18 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 22 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 26 },
				new LearnableMove { MoveId = "brace", LearnLevel = 30 },
				new LearnableMove { MoveId = "dread_gaze", LearnLevel = 32 },
				new LearnableMove { MoveId = "phantom_rend", LearnLevel = 34 },
				new LearnableMove { MoveId = "shadow_cloak", LearnLevel = 36 },
				new LearnableMove { MoveId = "oblivion_claw", LearnLevel = 38 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 40 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 42 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 43 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 22 },
				new LearnableMove { MoveId = "brace", LearnLevel = 23 },
				new LearnableMove { MoveId = "shadow_cloak", LearnLevel = 24 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 25 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 26 },
				new LearnableMove { MoveId = "hex_blast", LearnLevel = 27 },
				new LearnableMove { MoveId = "void_sphere", LearnLevel = 28 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 29 },
				new LearnableMove { MoveId = "shadow_wave", LearnLevel = 30 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 31 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "shade_step", LearnLevel = 1 },
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 5 },
				new LearnableMove { MoveId = "sly_strike", LearnLevel = 8 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 10 },
				new LearnableMove { MoveId = "umbral_claw", LearnLevel = 14 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 16 },
				new LearnableMove { MoveId = "shadow_lunge", LearnLevel = 18 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 20 },
				new LearnableMove { MoveId = "gloom_strike", LearnLevel = 22 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 24 },
				new LearnableMove { MoveId = "brace", LearnLevel = 26 },
				new LearnableMove { MoveId = "nightfall_rush", LearnLevel = 28 },
				new LearnableMove { MoveId = "shadow_cloak", LearnLevel = 30 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 32 },
				new LearnableMove { MoveId = "phantom_rend", LearnLevel = 34 },
				new LearnableMove { MoveId = "dread_gaze", LearnLevel = 36 },
				new LearnableMove { MoveId = "oblivion_claw", LearnLevel = 38 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "shade_step", LearnLevel = 1 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 5 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 8 },
				new LearnableMove { MoveId = "dusk_bolt", LearnLevel = 12 },
				new LearnableMove { MoveId = "dread_gaze", LearnLevel = 16 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 20 },
				new LearnableMove { MoveId = "shadow_cloak", LearnLevel = 22 },
				new LearnableMove { MoveId = "murk_bolt", LearnLevel = 24 },
				new LearnableMove { MoveId = "brace", LearnLevel = 26 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 28 },
				new LearnableMove { MoveId = "void_sphere", LearnLevel = 30 },
				new LearnableMove { MoveId = "hex_blast", LearnLevel = 32 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 34 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 36 },
				new LearnableMove { MoveId = "shadow_wave", LearnLevel = 38 },
				new LearnableMove { MoveId = "fated_curse", LearnLevel = 40 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 41 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "divine_grace", LearnLevel = 1 },
				new LearnableMove { MoveId = "shade_step", LearnLevel = 5 },
				new LearnableMove { MoveId = "aether_pulse", LearnLevel = 8 },
				new LearnableMove { MoveId = "dusk_bolt", LearnLevel = 12 },
				new LearnableMove { MoveId = "lunar_radiance", LearnLevel = 16 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 18 },
				new LearnableMove { MoveId = "spirit_breeze", LearnLevel = 22 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 26 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 28 },
				new LearnableMove { MoveId = "brace", LearnLevel = 30 },
				new LearnableMove { MoveId = "shadow_wave", LearnLevel = 32 },
				new LearnableMove { MoveId = "abyssal_torrent", LearnLevel = 34 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 36 },
				new LearnableMove { MoveId = "eclipse_ray", LearnLevel = 38 },
				new LearnableMove { MoveId = "hex_blast", LearnLevel = 40 },
				new LearnableMove { MoveId = "fate_glimpse", LearnLevel = 42 },
				new LearnableMove { MoveId = "void_pulse", LearnLevel = 44 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 46 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 1 },
				new LearnableMove { MoveId = "shade_step", LearnLevel = 5 },
				new LearnableMove { MoveId = "dusk_bolt", LearnLevel = 8 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 12 },
				new LearnableMove { MoveId = "dread_gaze", LearnLevel = 16 },
				...
				new LearnableMove { MoveId = "hex_blast", LearnLevel = 28 },
				new LearnableMove { MoveId = "abyssal_torrent", LearnLevel = 34 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 36 },
				new LearnableMove { MoveId = "shadow_wave", LearnLevel = 38 },
				new LearnableMove { MoveId = "fated_curse", LearnLevel = 40 },
				new LearnableMove { MoveId = "void_pulse", LearnLevel = 42 },
				new LearnableMove { MoveId = "eclipse_ray", LearnLevel = 44 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 46 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "vitality_burst", LearnLevel = 12 },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 16 },
				new LearnableMove { MoveId = "blossom_frenzy", LearnLevel = 20 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 24 },
				new LearnableMove { MoveId = "vine_crush", LearnLevel = 28 },
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 32 },
				new LearnableMove { MoveId = "brace", LearnLevel = 34 },
				new LearnableMove { MoveId = "thorn_barrage", LearnLevel = 36 },
				new LearnableMove { MoveId = "growth_surge", LearnLevel = 38 },
				new LearnableMove { MoveId = "photon_bloom", LearnLevel = 40 },
				new LearnableMove { MoveId = "bloom_burst", LearnLevel = 45 },
				new LearnableMove { MoveId = "canopy_crash", LearnLevel = 48 },
				new LearnableMove { MoveId = "evergreen_pulse", LearnLevel = 50 },
				new LearnableMove { MoveId = "jungle_wrath", LearnLevel = 55 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 5 },
				new LearnableMove { MoveId = "harden", LearnLevel = 8 },
				new LearnableMove { MoveId = "scorch_wave", LearnLevel = 12 },
				new LearnableMove { MoveId = "molten_ray", LearnLevel = 16 },
				new LearnableMove { MoveId = "heat_haze", LearnLevel = 20 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 24 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 28 },
				new LearnableMove { MoveId = "lava_plume", LearnLevel = 30 },
				new LearnableMove { MoveId = "brace", LearnLevel = 32 },
				new LearnableMove { MoveId = "conflagration", LearnLevel = 34 },
				new LearnableMove { MoveId = "scorching_gust", LearnLevel = 36 },
				new LearnableMove { MoveId = "flame_vortex", LearnLevel = 40 },
				new LearnableMove { MoveId = "volcanic_fury", LearnLevel = 42 },
				new LearnableMove { MoveId = "rift_flame", LearnLevel = 44 },
				new LearnableMove { MoveId = "solar_flare", LearnLevel = 48 },
				new LearnableMove { MoveId = "primordial_flame", LearnLevel = 52 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "cryo_beam", LearnLevel = 5 },
				new LearnableMove { MoveId = "ice_armor", LearnLevel = 10 },
				new LearnableMove { MoveId = "deep_freeze", LearnLevel = 14 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 18 },
				new LearnableMove { MoveId = "permafrost_ray", LearnLevel = 22 },
				new LearnableMove { MoveId = "glacial_mirror", LearnLevel = 26 },
				new LearnableMove { MoveId = "blizzard_wrath", LearnLevel = 30 },
				new LearnableMove { MoveId = "brace", LearnLevel = 33 },
				new LearnableMove { MoveId = "frost_crush", LearnLevel = 36 },
				new LearnableMove { MoveId = "frostbite_nova", LearnLevel = 40 },
				new LearnableMove { MoveId = "subzero_pulse", LearnLevel = 44 },
				new LearnableMove { MoveId = "crystallize", LearnLevel = 46 },
				new LearnableMove { MoveId = "absolute_zero", LearnLevel = 50 },
				new LearnableMove { MoveId = "glacier_slam", LearnLevel = 54 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 58 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 62 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "chain_lightning", LearnLevel = 5 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 10 },
				new LearnableMove { MoveId = "surge_field", LearnLevel = 14 },
				new LearnableMove { MoveId = "charge_up", LearnLevel = 18 },
				new LearnableMove { MoveId = "arc_bolt", LearnLevel = 22 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 26 },
				new LearnableMove { MoveId = "plasma_charge", LearnLevel = 30 },
				new LearnableMove { MoveId = "brace", LearnLevel = 33 },
				...
				new LearnableMove { MoveId = "tempest", LearnLevel = 36 },
				new LearnableMove { MoveId = "overcharge", LearnLevel = 40 },
				new LearnableMove { MoveId = "megavolt", LearnLevel = 44 },
				new LearnableMove { MoveId = "storm_surge", LearnLevel = 48 },
				new LearnableMove { MoveId = "galvanic_shield", LearnLevel = 52 },
				new LearnableMove { MoveId = "ion_cannon", LearnLevel = 56 },
				new LearnableMove { MoveId = "discharge_pulse", LearnLevel = 60 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "intimidate", LearnLevel = 10 },
				new LearnableMove { MoveId = "harden", LearnLevel = 16 },
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 20 },
				new LearnableMove { MoveId = "energy_pulse", LearnLevel = 24 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 28 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 30 },
				new LearnableMove { MoveId = "brace", LearnLevel = 34 },
				new LearnableMove { MoveId = "resonant_howl", LearnLevel = 36 },
				new LearnableMove { MoveId = "wild_surge", LearnLevel = 38 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 40 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 44 },
				new LearnableMove { MoveId = "primal_roar", LearnLevel = 48 },
				new LearnableMove { MoveId = "primordial_surge", LearnLevel = 52 },
				new LearnableMove { MoveId = "oblivion_ray", LearnLevel = 56 },
				new LearnableMove { MoveId = "cataclysm", LearnLevel = 60 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "temper", LearnLevel = 1 },
				new LearnableMove { MoveId = "tectonic_slam", LearnLevel = 5 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 10 },
				new LearnableMove { MoveId = "fissure_wave", LearnLevel = 15 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 20 },
				new LearnableMove { MoveId = "continental_crush", LearnLevel = 25 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 30 },
				new LearnableMove { MoveId = "landslide", LearnLevel = 35 },
				new LearnableMove { MoveId = "primeval_force", LearnLevel = 38 },
				new LearnableMove { MoveId = "tidal_slam", LearnLevel = 42 },
				new LearnableMove { MoveId = "quake_stomp", LearnLevel = 46 },
				new LearnableMove { MoveId = "petrify", LearnLevel = 50 },
				new LearnableMove { MoveId = "brace", LearnLevel = 52 },
				new LearnableMove { MoveId = "mineral_lance", LearnLevel = 55 },
				new LearnableMove { MoveId = "world_crush", LearnLevel = 60 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 64 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "shade_step", LearnLevel = 5 },
				new LearnableMove { MoveId = "dusk_bolt", LearnLevel = 10 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 16 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 20 },
				new LearnableMove { MoveId = "shadow_lunge", LearnLevel = 24 },
				new LearnableMove { MoveId = "dread_gaze", LearnLevel = 28 },
				new LearnableMove { MoveId = "terror_visions", LearnLevel = 32 },
				new LearnableMove { MoveId = "brace", LearnLevel = 34 },
				new LearnableMove { MoveId = "abyssal_torrent", LearnLevel = 38 },
				new LearnableMove { MoveId = "shadow_wave", LearnLevel = 40 },
				new LearnableMove { MoveId = "hex_blast", LearnLevel = 42 },
				new LearnableMove { MoveId = "void_tear", LearnLevel = 44 },
				new LearnableMove { MoveId = "fated_curse", LearnLevel = 46 },
				new LearnableMove { MoveId = "void_wings", LearnLevel = 48 },
				new LearnableMove { MoveId = "eclipse_ray", LearnLevel = 52 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 56 },
				new LearnableMove { MoveId = "absolute_void", LearnLevel = 60 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "intimidate", LearnLevel = 10 },
				new LearnableMove { MoveId = "harden", LearnLevel = 16 },
				new LearnableMove { MoveId = "energy_pulse", LearnLevel = 20 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 24 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 32 },
				new LearnableMove { MoveId = "wild_surge", LearnLevel = 36 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 40 },
				new LearnableMove { MoveId = "resonant_howl", LearnLevel = 44 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 48 },
				new LearnableMove { MoveId = "primal_roar", LearnLevel = 52 },
				new LearnableMove { MoveId = "primordial_surge", LearnLevel = 56 },
				new LearnableMove { MoveId = "oblivion_ray", LearnLevel = 60 },
				new LearnableMove { MoveId = "genesis_wave", LearnLevel = 64 },
				new LearnableMove { MoveId = "cataclysm", LearnLevel = 68 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 10 },
				new LearnableMove { MoveId = "bolster", LearnLevel = 16 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 20 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 24 },
				new LearnableMove { MoveId = "lunar_radiance", LearnLevel = 28 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 32 },
				new LearnableMove { MoveId = "spectral_rush", LearnLevel = 36 },
				new LearnableMove { MoveId = "brace", LearnLevel = 38 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 40 },
				new LearnableMove { MoveId = "fate_glimpse", LearnLevel = 42 },
				new LearnableMove { MoveId = "radiant_burst", LearnLevel = 46 },
				new LearnableMove { MoveId = "aurora_wave", LearnLevel = 50 },
				new LearnableMove { MoveId = "celestial_flare", LearnLevel = 54 },
				new LearnableMove { MoveId = "judgment_blow", LearnLevel = 58 },
				new LearnableMove { MoveId = "genesis_thought", LearnLevel = 60 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 64 },
				new LearnableMove { MoveId = "universe_burst", LearnLevel = 70 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "cinders_curse", LearnLevel = 1 },
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 5 },
				new LearnableMove { MoveId = "scorch_wave", LearnLevel = 10 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 14 },
				new LearnableMove { MoveId = "molten_ray", LearnLevel = 18 },
				new LearnableMove { MoveId = "flame_vortex", LearnLevel = 22 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 26 },
				new LearnableMove { MoveId = "ash_cloud", LearnLevel = 30 },
				new LearnableMove { MoveId = "brace", LearnLevel = 32 },
				new LearnableMove { MoveId = "lava_plume", LearnLevel = 34 },
				new LearnableMove { MoveId = "scorching_gust", LearnLevel = 38 },
				new LearnableMove { MoveId = "volcanic_fury", LearnLevel = 42 },
				new LearnableMove { MoveId = "solar_flare", LearnLevel = 44 },
				new LearnableMove { MoveId = "rift_flame", LearnLevel = 46 },
				new LearnableMove { MoveId = "conflagration", LearnLevel = 48 },
				new LearnableMove { MoveId = "primordial_flame", LearnLevel = 50 },
				new LearnableMove { MoveId = "wildfire_burst", LearnLevel = 54 },
				new LearnableMove { MoveId = "pyroclasm", LearnLevel = 58 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 62 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "shade_step", LearnLevel = 5 },
				new LearnableMove { MoveId = "thorn_lash", LearnLevel = 8 },
				new LearnableMove { MoveId = "dusk_bolt", LearnLevel = 12 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 16 },
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 20 },
				new LearnableMove { MoveId = "nightmare_wave", LearnLevel = 24 },
				new LearnableMove { MoveId = "vitality_burst", LearnLevel = 28 },
				new LearnableMove { MoveId = "brace", LearnLevel = 30 },
				new LearnableMove { MoveId = "shadow_wave", LearnLevel = 32 },
				...
				new LearnableMove { MoveId = "hex_blast", LearnLevel = 40 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 42 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 44 },
				new LearnableMove { MoveId = "void_bloom", LearnLevel = 48 },
				new LearnableMove { MoveId = "fated_curse", LearnLevel = 50 },
				new LearnableMove { MoveId = "eclipse_ray", LearnLevel = 52 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 55 },
			},
			AnimationFrames = new()
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "divine_grace", LearnLevel = 26 },
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "floral_guard", LearnLevel = 30 },
				new LearnableMove { MoveId = "growth_surge", LearnLevel = 32 },
				new LearnableMove { MoveId = "tangling_roots", LearnLevel = 34 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 36 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 38 },
				new LearnableMove { MoveId = "evergreen_pulse", LearnLevel = 40 },
				new LearnableMove { MoveId = "canopy_crash", LearnLevel = 42 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 44 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "harden", LearnLevel = 5 },
				new LearnableMove { MoveId = "water_bolt", LearnLevel = 8 },
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 12 },
				...
				new LearnableMove { MoveId = "crashing_wave", LearnLevel = 32 },
				new LearnableMove { MoveId = "sea_chant", LearnLevel = 34 },
				new LearnableMove { MoveId = "ocean_torrent", LearnLevel = 36 },
				new LearnableMove { MoveId = "maelstrom_slam", LearnLevel = 38 },
				new LearnableMove { MoveId = "deluge", LearnLevel = 40 },
				new LearnableMove { MoveId = "monsoon_call", LearnLevel = 45 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "spark_claw", LearnLevel = 3 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 5 },
				new LearnableMove { MoveId = "stun_wave", LearnLevel = 7 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 9 },
				new LearnableMove { MoveId = "volt_charge", LearnLevel = 10 },
				new LearnableMove { MoveId = "charge_up", LearnLevel = 12 },
				new LearnableMove { MoveId = "spark_wave", LearnLevel = 14 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 16 },
				new LearnableMove { MoveId = "bolt_rush", LearnLevel = 18 },
				new LearnableMove { MoveId = "static_field", LearnLevel = 19 },
				new LearnableMove { MoveId = "shock_tackle", LearnLevel = 20 },
				new LearnableMove { MoveId = "arc_bolt", LearnLevel = 22 },
				new LearnableMove { MoveId = "harden", LearnLevel = 23 },
				new LearnableMove { MoveId = "voltage_ray", LearnLevel = 24 },
				new LearnableMove { MoveId = "nerve_lock", LearnLevel = 25 },
				new LearnableMove { MoveId = "lightning_fang", LearnLevel = 26 },
				new LearnableMove { MoveId = "circuit_burst", LearnLevel = 28 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "gravel_toss", LearnLevel = 3 },
				new LearnableMove { MoveId = "mud_hurl", LearnLevel = 5 },
				new LearnableMove { MoveId = "sand_blast", LearnLevel = 7 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 9 },
				new LearnableMove { MoveId = "sandstone_strike", LearnLevel = 11 },
				new LearnableMove { MoveId = "intimidate", LearnLevel = 13 },
				new LearnableMove { MoveId = "seismic_crash", LearnLevel = 14 },
				new LearnableMove { MoveId = "terra_pulse", LearnLevel = 15 },
				new LearnableMove { MoveId = "dust_storm", LearnLevel = 17 },
				new LearnableMove { MoveId = "bedrock_stance", LearnLevel = 19 },
				new LearnableMove { MoveId = "earthen_armor", LearnLevel = 21 },
				new LearnableMove { MoveId = "tremor_pulse", LearnLevel = 23 },
				new LearnableMove { MoveId = "quicksand", LearnLevel = 24 },
				new LearnableMove { MoveId = "jagged_spike", LearnLevel = 26 },
				new LearnableMove { MoveId = "stone_wall", LearnLevel = 28 },
				new LearnableMove { MoveId = "fault_breaker", LearnLevel = 30 },
				new LearnableMove { MoveId = "tectonic_slam", LearnLevel = 34 },
				new LearnableMove { MoveId = "brace", LearnLevel = 38 }
			},
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 8 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 10 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 12 },
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 14 },
				new LearnableMove { MoveId = "gust_shield", LearnLevel = 16 },
				new LearnableMove { MoveId = "vortex_blast", LearnLevel = 18 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 20 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 22 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 24 },
				new LearnableMove { MoveId = "tempest", LearnLevel = 26 },
				new LearnableMove { MoveId = "wind_wall", LearnLevel = 28 },
				new LearnableMove { MoveId = "squall_burst", LearnLevel = 30 },
				new LearnableMove { MoveId = "tailwind_boost", LearnLevel = 32 },
				new LearnableMove { MoveId = "tornado_dive", LearnLevel = 35 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "quantum_flux", LearnLevel = 34 },
				new LearnableMove { MoveId = "storm_strike", LearnLevel = 38 },
				new LearnableMove { MoveId = "megavolt", LearnLevel = 42 },
				new LearnableMove { MoveId = "galvanic_shield", LearnLevel = 44 },
				new LearnableMove { MoveId = "ion_cannon", LearnLevel = 46 },
				new LearnableMove { MoveId = "storm_surge", LearnLevel = 48 },
				new LearnableMove { MoveId = "dimensional_rift", LearnLevel = 50 },
				new LearnableMove { MoveId = "overload_crash", LearnLevel = 54 },
				new LearnableMove { MoveId = "discharge_pulse", LearnLevel = 58 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 62 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "spark_wave", LearnLevel = 4 },
				new LearnableMove { MoveId = "charge_up", LearnLevel = 8 },
				new LearnableMove { MoveId = "stun_wave", LearnLevel = 12 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 15 },
				new LearnableMove { MoveId = "arc_bolt", LearnLevel = 18 },
				new LearnableMove { MoveId = "static_field", LearnLevel = 20 },
				new LearnableMove { MoveId = "bolt_rush", LearnLevel = 22 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 24 },
				new LearnableMove { MoveId = "chain_lightning", LearnLevel = 26 },
				new LearnableMove { MoveId = "spirit_rend", LearnLevel = 28 },
				new LearnableMove { MoveId = "thunder_fang", LearnLevel = 30 },
				new LearnableMove { MoveId = "plasma_charge", LearnLevel = 32 },
				new LearnableMove { MoveId = "overcharge", LearnLevel = 34 },
				new LearnableMove { MoveId = "brace", LearnLevel = 36 },
				new LearnableMove { MoveId = "overload_crash", LearnLevel = 38 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "dark_scheme", LearnLevel = 20 },
				new LearnableMove { MoveId = "harden", LearnLevel = 24 },
				new LearnableMove { MoveId = "abyssal_torrent", LearnLevel = 28 },
				new LearnableMove { MoveId = "brace", LearnLevel = 30 },
				new LearnableMove { MoveId = "shadow_wave", LearnLevel = 32 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 34 },
				new LearnableMove { MoveId = "hex_blast", LearnLevel = 36 },
				new LearnableMove { MoveId = "fated_curse", LearnLevel = 38 },
				new LearnableMove { MoveId = "void_tear", LearnLevel = 40 },
				new LearnableMove { MoveId = "shadow_cloak", LearnLevel = 42 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 44 },
				new LearnableMove { MoveId = "eclipse_ray", LearnLevel = 46 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "breeze_cut", LearnLevel = 1 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 1 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 4 },
				new LearnableMove { MoveId = "gust_claw", LearnLevel = 6 },
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 8 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 10 },
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 12 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 14 },
				new LearnableMove { MoveId = "razor_gale", LearnLevel = 16 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 18 },
				new LearnableMove { MoveId = "shade_step", LearnLevel = 20 },
				new LearnableMove { MoveId = "vortex_blast", LearnLevel = 22 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 24 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 26 },
				new LearnableMove { MoveId = "phase_shift", LearnLevel = 28 },
				new LearnableMove { MoveId = "tempest", LearnLevel = 30 },
				new LearnableMove { MoveId = "wind_wall", LearnLevel = 32 },
				new LearnableMove { MoveId = "tornado_dive", LearnLevel = 35 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "ember_claw", LearnLevel = 14 },
				new LearnableMove { MoveId = "heat_haze", LearnLevel = 16 },
				new LearnableMove { MoveId = "scorch_wave", LearnLevel = 18 },
				new LearnableMove { MoveId = "flame_vortex", LearnLevel = 20 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 22 },
				new LearnableMove { MoveId = "molten_ray", LearnLevel = 24 },
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 26 },
				new LearnableMove { MoveId = "cinders_curse", LearnLevel = 28 },
				new LearnableMove { MoveId = "rift_flame", LearnLevel = 30 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 33 },
				new LearnableMove { MoveId = "blazing_wrath", LearnLevel = 36 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "astral_ward", LearnLevel = 1 },
				new LearnableMove { MoveId = "strike", LearnLevel = 5 },
				new LearnableMove { MoveId = "rend", LearnLevel = 8 },
				new LearnableMove { MoveId = "harden", LearnLevel = 12 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 16 },
				new LearnableMove { MoveId = "feral_charge", LearnLevel = 20 },
				new LearnableMove { MoveId = "brace", LearnLevel = 24 },
				new LearnableMove { MoveId = "savage_rend", LearnLevel = 28 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 32 },
				new LearnableMove { MoveId = "primal_crash", LearnLevel = 36 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 40 },
				new LearnableMove { MoveId = "blunt_slam", LearnLevel = 44 },
				new LearnableMove { MoveId = "primal_roar", LearnLevel = 48 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 52 },
				new LearnableMove { MoveId = "resonant_howl", LearnLevel = 56 },
				new LearnableMove { MoveId = "wild_surge", LearnLevel = 60 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "brace", LearnLevel = 28 },
				new LearnableMove { MoveId = "hex_blast", LearnLevel = 30 },
				new LearnableMove { MoveId = "shadow_wave", LearnLevel = 32 },
				new LearnableMove { MoveId = "curse_mark", LearnLevel = 34 },
				new LearnableMove { MoveId = "abyssal_torrent", LearnLevel = 38 },
				new LearnableMove { MoveId = "void_pulse", LearnLevel = 40 },
				new LearnableMove { MoveId = "fated_curse", LearnLevel = 42 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 44 },
				new LearnableMove { MoveId = "eclipse_ray", LearnLevel = 46 },
				new LearnableMove { MoveId = "void_tear", LearnLevel = 48 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "soul_siphon", LearnLevel = 10 },
				new LearnableMove { MoveId = "inner_focus", LearnLevel = 16 },
				new LearnableMove { MoveId = "bolster", LearnLevel = 20 },
				new LearnableMove { MoveId = "lunar_radiance", LearnLevel = 24 },
				new LearnableMove { MoveId = "luminous_wall", LearnLevel = 28 },
				new LearnableMove { MoveId = "sacred_strike", LearnLevel = 32 },
				new LearnableMove { MoveId = "fate_glimpse", LearnLevel = 36 },
				new LearnableMove { MoveId = "brace", LearnLevel = 38 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 40 },
				new LearnableMove { MoveId = "spectral_rush", LearnLevel = 42 },
				new LearnableMove { MoveId = "radiant_burst", LearnLevel = 46 },
				new LearnableMove { MoveId = "aurora_wave", LearnLevel = 50 },
				new LearnableMove { MoveId = "celestial_flare", LearnLevel = 54 },
				new LearnableMove { MoveId = "judgment_blow", LearnLevel = 58 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 62 },
				new LearnableMove { MoveId = "universe_burst", LearnLevel = 66 },
				new LearnableMove { MoveId = "creation_burst", LearnLevel = 70 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "spore_cloud", LearnLevel = 10 },
				new LearnableMove { MoveId = "harden", LearnLevel = 12 },
				new LearnableMove { MoveId = "parasite_spore", LearnLevel = 14 },
				new LearnableMove { MoveId = "vitality_burst", LearnLevel = 16 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 18 },
				new LearnableMove { MoveId = "toxic_pollen", LearnLevel = 20 },
				new LearnableMove { MoveId = "brace", LearnLevel = 22 },
				new LearnableMove { MoveId = "pollen_burst", LearnLevel = 24 },
				new LearnableMove { MoveId = "regenerate", LearnLevel = 26 },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 28 },
				new LearnableMove { MoveId = "growth_surge", LearnLevel = 30 },
				new LearnableMove { MoveId = "floral_guard", LearnLevel = 32 },
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "swift_lunge", LearnLevel = 1 },
				new LearnableMove { MoveId = "feint_jab", LearnLevel = 3 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 5 },
				new LearnableMove { MoveId = "kindle", LearnLevel = 6 },
				new LearnableMove { MoveId = "ember_claw", LearnLevel = 8 },
				new LearnableMove { MoveId = "smolder", LearnLevel = 10 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 12 },
				new LearnableMove { MoveId = "searing_rush", LearnLevel = 14 },
				new LearnableMove { MoveId = "flame_tackle", LearnLevel = 16 },
				new LearnableMove { MoveId = "pyre_fangs", LearnLevel = 17 },
				new LearnableMove { MoveId = "heat_haze", LearnLevel = 18 },
				new LearnableMove { MoveId = "flame_vortex", LearnLevel = 20 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 21 },
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 22 },
				new LearnableMove { MoveId = "scorch_wave", LearnLevel = 23 },
				new LearnableMove { MoveId = "sharp_focus", LearnLevel = 24 },
				new LearnableMove { MoveId = "singe_breath", LearnLevel = 25 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "ember_claw", LearnLevel = 1 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 5 },
				new LearnableMove { MoveId = "pyre_fangs", LearnLevel = 8 },
				new LearnableMove { MoveId = "phantom_double", LearnLevel = 12 },
				new LearnableMove { MoveId = "flame_tackle", LearnLevel = 14 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 16 },
				new LearnableMove { MoveId = "flame_shroud", LearnLevel = 18 },
				new LearnableMove { MoveId = "flame_vortex", LearnLevel = 20 },
				new LearnableMove { MoveId = "vicious_cut", LearnLevel = 22 },
				new LearnableMove { MoveId = "magma_fang", LearnLevel = 24 },
				new LearnableMove { MoveId = "cinders_curse", LearnLevel = 26 },
				new LearnableMove { MoveId = "eruption_slam", LearnLevel = 28 },
				new LearnableMove { MoveId = "blazing_wrath", LearnLevel = 30 },
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 34 },
				new LearnableMove { MoveId = "ash_cloud", LearnLevel = 36 },
				new LearnableMove { MoveId = "inferno_blitz", LearnLevel = 40 },
				new LearnableMove { MoveId = "conflagration", LearnLevel = 44 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 1 },
				new LearnableMove { MoveId = "harden", LearnLevel = 5 },
				new LearnableMove { MoveId = "crashing_wave", LearnLevel = 10 },
				new LearnableMove { MoveId = "hydro_fang", LearnLevel = 15 },
				new LearnableMove { MoveId = "brace", LearnLevel = 18 },
				new LearnableMove { MoveId = "maelstrom_slam", LearnLevel = 22 },
				new LearnableMove { MoveId = "deep_pressure", LearnLevel = 26 },
				new LearnableMove { MoveId = "aqua_shield", LearnLevel = 30 },
				new LearnableMove { MoveId = "primal_roar", LearnLevel = 34 },
				new LearnableMove { MoveId = "ocean_torrent", LearnLevel = 38 },
				new LearnableMove { MoveId = "deluge", LearnLevel = 40 },
				new LearnableMove { MoveId = "tsunami_crush", LearnLevel = 45 },
				new LearnableMove { MoveId = "sea_chant", LearnLevel = 48 },
				new LearnableMove { MoveId = "undertow_strike", LearnLevel = 52 },
				new LearnableMove { MoveId = "abyssal_wave", LearnLevel = 56 },
				new LearnableMove { MoveId = "seismic_surge", LearnLevel = 60 },
				new LearnableMove { MoveId = "cataclysm", LearnLevel = 65 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 70 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "crushing_blow", LearnLevel = 1 },
				new LearnableMove { MoveId = "strike", LearnLevel = 5 },
				new LearnableMove { MoveId = "steel_rake", LearnLevel = 10 },
				new LearnableMove { MoveId = "temper", LearnLevel = 16 },
				new LearnableMove { MoveId = "burnish", LearnLevel = 20 },
				new LearnableMove { MoveId = "titan_drop", LearnLevel = 28 },
				new LearnableMove { MoveId = "brace", LearnLevel = 32 },
				new LearnableMove { MoveId = "starfall_strike", LearnLevel = 36 },
				new LearnableMove { MoveId = "iron_rebound", LearnLevel = 40 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 44 },
				new LearnableMove { MoveId = "foundry_crash", LearnLevel = 48 },
				new LearnableMove { MoveId = "forge_blast", LearnLevel = 52 },
				new LearnableMove { MoveId = "steel_rend", LearnLevel = 56 },
				new LearnableMove { MoveId = "molten_vein", LearnLevel = 60 },
				new LearnableMove { MoveId = "alloy_smash", LearnLevel = 64 },
				new LearnableMove { MoveId = "primal_roar", LearnLevel = 66 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 68 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 70 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "dive_strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "wind_blade", LearnLevel = 5 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 8 },
				new LearnableMove { MoveId = "storm_talon", LearnLevel = 12 },
				new LearnableMove { MoveId = "vortex_blast", LearnLevel = 16 },
				new LearnableMove { MoveId = "sky_shear", LearnLevel = 20 },
				new LearnableMove { MoveId = "cyclone_tackle", LearnLevel = 24 },
				new LearnableMove { MoveId = "howling_gust", LearnLevel = 28 },
				new LearnableMove { MoveId = "hurricane_slam", LearnLevel = 32 },
				new LearnableMove { MoveId = "slipstream", LearnLevel = 36 },
				new LearnableMove { MoveId = "gale_slam", LearnLevel = 40 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 44 },
				new LearnableMove { MoveId = "typhoon_wrath", LearnLevel = 48 },
				new LearnableMove { MoveId = "tornado_dive", LearnLevel = 52 },
				new LearnableMove { MoveId = "windshear", LearnLevel = 55 },
				new LearnableMove { MoveId = "tempest_unleash", LearnLevel = 60 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 65 },
				new LearnableMove { MoveId = "jet_stream", LearnLevel = 70 },
			},
			AnimationFrames = new()
			{
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "cryo_beam", LearnLevel = 5 },
				new LearnableMove { MoveId = "ice_armor", LearnLevel = 10 },
				new LearnableMove { MoveId = "deep_freeze", LearnLevel = 15 },
				new LearnableMove { MoveId = "blizzard_wrath", LearnLevel = 20 },
				new LearnableMove { MoveId = "glacial_mirror", LearnLevel = 25 },
				new LearnableMove { MoveId = "frostbite_nova", LearnLevel = 30 },
				new LearnableMove { MoveId = "brace", LearnLevel = 34 },
				new LearnableMove { MoveId = "subzero_pulse", LearnLevel = 38 },
				new LearnableMove { MoveId = "avalanche_wrath", LearnLevel = 40 },
				new LearnableMove { MoveId = "crystallize", LearnLevel = 44 },
				new LearnableMove { MoveId = "frost_crush", LearnLevel = 48 },
				new LearnableMove { MoveId = "glacier_slam", LearnLevel = 52 },
				new LearnableMove { MoveId = "war_cry", LearnLevel = 55 },
				new LearnableMove { MoveId = "cold_shoulder", LearnLevel = 60 },
				new LearnableMove { MoveId = "absolute_zero", LearnLevel = 65 },
				new LearnableMove { MoveId = "annihilate", LearnLevel = 68 },
				new LearnableMove { MoveId = "cataclysm", LearnLevel = 72 }
			},
			AnimationFrames = new()
			{
		...
		// ═══════════════════════════════════════════════════════════════
		// SALTMOOR COVE — extra zone-1 wilds (#10-12)
		// Three handmade beasts in the starter expedition pool. Slotted
		// at 10/11/12 right after the starters; the rest of the roster
		// was bumped +3 to make room.
		// ═══════════════════════════════════════════════════════════════

		// Wishlift — based on the faerie myth (carried by dream-bubbles)
		AddSpecies( new MonsterSpecies
		{
			Id = "wishlift",
			Name = "Wishlift",
			...
			AnimationFrames = new()
			{
				"ui/monsters/wishlift/idle/wishlift_idle_01.png"
			},
			AnimationFrameRate = 8f,
			...
			Personality = BeastPersonality.Timid,
			PersonalityHint = "She tucks behind her bubbles when you draw near, peeking through the largest one as if it were a window.",
			PossibleTraits = new() { "gale_spirit", "vital_recovery", "subtle_arts" },
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "breeze_cut", LearnLevel = 1 },
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "spirit_breeze", LearnLevel = 4 },
				new LearnableMove { MoveId = "calm_air", LearnLevel = 6 },
				new LearnableMove { MoveId = "draft_rush", LearnLevel = 8 },
				new LearnableMove { MoveId = "updraft", LearnLevel = 10 },
				new LearnableMove { MoveId = "gust_shield", LearnLevel = 12 },
				new LearnableMove { MoveId = "agility_boost", LearnLevel = 14 },
			...
			BeastiaryNumber = 12,
			CardScale = 1f, CardOffsetX = 20f, CardOffsetY = 2f,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "calm_air", LearnLevel = 1 },
				new LearnableMove { MoveId = "spirit_breeze", LearnLevel = 5 },
		...
		// Twincoil — based on the two-faced (Janus) myth, garden snake
		AddSpecies( new MonsterSpecies
		{
			Id = "twincoil",
			Name = "Twincoil",
			...
			AnimationFrames = new()
			{
				// Static on the mouth-open pose so the second pair of eyes
				// inside the mouth always reads. The closed-mouth frame
				// (twincoil_idle_01.png) stays in the folder for if/when
				// we revisit the reveal animation.
				"ui/monsters/twincoil/idle/twincoil_idle_02.png"
			},
			AnimationFrameRate = 8f,
			BaseHP = 50, BaseATK = 56, BaseDEF = 46, BaseSpA = 42, BaseSpD = 45, BaseSPD = 51,
			HPGrowth = 5, ATKGrowth = 5, DEFGrowth = 5, SpAGrowth = 4, SpDGrowth = 5, SPDGrowth = 6,
			Element = ElementType.Nature,
			...
			Personality = BeastPersonality.Greedy,
			PersonalityHint = "Its outer eyes appraise the bowl of milk. Then its jaws part — and the eyes inside its mouth appraise you. Neither pair blinks.",
			PossibleTraits = new() { "verdant_power", "barbed_hide", "trickster" },
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "thorn_lash", LearnLevel = 1 },
				new LearnableMove { MoveId = "rend", LearnLevel = 1 },
			...
			BeastiaryNumber = 14,
			CardScale = 1f, CardOffsetX = 8f, CardOffsetY = 4f,
		...
		// Heartwell — based on sokushinbutsu (self-mummified monk)
		AddSpecies( new MonsterSpecies
		{
			Id = "heartwell",
			Name = "Heartwell",
			Description = "A monk who sat so long beneath the village fountain that the spring took him in. He still keeps his vows, channeling cold, clean water through the gardens — villagers call to him when the shallow wells run dry.",
			IconPath = "ui/monsters/heartwell/idle/heartwell_idle_01.png",
			AnimationFrames = new()
			{
				"ui/monsters/heartwell/idle/heartwell_idle_01.png"
			},
			AnimationFrameRate = 8f,
			BaseHP = 65, BaseATK = 30, BaseDEF = 52, BaseSpA = 52, BaseSpD = 68, BaseSPD = 30,
			HPGrowth = 6, ATKGrowth = 3, DEFGrowth = 5, SpAGrowth = 5, SpDGrowth = 7, SPDGrowth = 4,
			Element = ElementType.Water,
			BaseRarity = Rarity.Common,
			BaseCatchRate = 0.65f,
			BaseExpYield = 75,
			...
			Personality = BeastPersonality.Loyal,
			PersonalityHint = "He does not turn to face you. The water around him simply holds its breath until you have finished speaking.",
			PossibleTraits = new() { "torrent_soul", "vital_recovery", "cleansing_retreat" },
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "splash_jet", LearnLevel = 1 },
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "drizzle_mist", LearnLevel = 4 },
				new LearnableMove { MoveId = "harden", LearnLevel = 6 },
				new LearnableMove { MoveId = "foam_coat", LearnLevel = 8 },
				new LearnableMove { MoveId = "aqua_shield", LearnLevel = 10 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 12 },
				new LearnableMove { MoveId = "deep_slumber", LearnLevel = 14 },
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 16 },
				new LearnableMove { MoveId = "warding_hymn", LearnLevel = 18 },
				...
				new LearnableMove { MoveId = "deep_pressure", LearnLevel = 24 },
				new LearnableMove { MoveId = "spirit_link", LearnLevel = 25 },
			},
			BeastiaryNumber = 15,
			CardScale = 1f, CardOffsetX = 7f, CardOffsetY = 1f,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 8 },
				new LearnableMove { MoveId = "parasite_spore", LearnLevel = 10 },
				new LearnableMove { MoveId = "root_bind", LearnLevel = 12 },
			...
			BeastiaryNumber = 10,
			CardScale = 1.25f, CardOffsetX = 6f, CardOffsetY = 34f,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 40 },
				new LearnableMove { MoveId = "verdant_edge", LearnLevel = 44 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 48 },
			},
			BeastiaryNumber = 11,
			CardScale = 1.2f, CardOffsetX = 10f, CardOffsetY = 39f,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BeastiaryNumber = 16,
			CardScale = 1.2f, CardOffsetX = 6f, CardOffsetY = 34f,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "pollen_burst", LearnLevel = 1, EvolvesFrom = "thorn_barrage" },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 1 },
				new LearnableMove { MoveId = "root_bind", LearnLevel = 1 },
				new LearnableMove { MoveId = "harden", LearnLevel = 1 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 5 },
				...
				new LearnableMove { MoveId = "verdant_edge", LearnLevel = 42 },
				new LearnableMove { MoveId = "solstice_beam", LearnLevel = 46 },
			},
			BeastiaryNumber = 17,
			CardScale = 1.2f, CardOffsetX = 0f, CardOffsetY = 26f,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			BeastiaryNumber = 20,
			CardScale = 1.45f, CardOffsetX = 1f, CardOffsetY = 62f,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "foam_coat", LearnLevel = 16 },
				new LearnableMove { MoveId = "ripple_burst", LearnLevel = 18 },
				new LearnableMove { MoveId = "tidal_pulse", LearnLevel = 20 },
				new LearnableMove { MoveId = "mist_veil", LearnLevel = 22 },
				new LearnableMove { MoveId = "deep_pressure", LearnLevel = 24 },
				new LearnableMove { MoveId = "aqua_shield", LearnLevel = 26 },
				new LearnableMove { MoveId = "waterlog", LearnLevel = 28 },
				new LearnableMove { MoveId = "endure_stance", LearnLevel = 32 },
			},
			BeastiaryNumber = 21,
			CardScale = 1.3f, CardOffsetX = -1f, CardOffsetY = 51f,
		...
		AddSpecies( new MonsterSpecies
		{
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "deluge", LearnLevel = 42 },
			...
			BeastiaryNumber = 22,
			CardScale = 1.35f, CardOffsetX = 4f, CardOffsetY = 58f,
		...
		// Stomplet — the young bigfoot shepherd of the Weaverwood. He
		// tends the Sheepot herds on the forest road; his footprints sink
		// deep but the flock follows. Cryptid subtext (small bigfoot)
		// stays implicit; in-world he reads as a leafy young giant.
		AddSpecies( new MonsterSpecies
		{
			Id = "stomplet",
			Name = "Stomplet",
			Description = "A leafy young giant who tends the Sheepot herds along the forest road. His footprints sink deep, but he walks lightly enough that the flock keeps following.",
			IconPath = "ui/monsters/stomplet/idle/stomplet_idle_01.png",
			ArtistCredit = "Jet",
			BaseHP = 70, BaseATK = 58, BaseDEF = 65, BaseSpA = 45, BaseSpD = 52, BaseSPD = 40,
			HPGrowth = 5, ATKGrowth = 5, DEFGrowth = 6, SpAGrowth = 4, SpDGrowth = 6, SPDGrowth = 6,
			Element = ElementType.Nature,
			BaseRarity = Rarity.Uncommon,
			EvolvesTo = "skunkape",
			EvolutionLevel = 32,
			BaseCatchRate = 0.32f,
			BaseExpYield = 145,
			SignatureDropName = "Mossy Tuft",
			SignatureDropDescription = "A tuft of green-mossy hair shed from a stomplet. Smells faintly of damp leaves and Sheepot wool.",
			Personality = BeastPersonality.Timid,
			PersonalityHint = "He turns his face slightly away when you draw close, as if embarrassed to have been noticed at all.",
			...
			LearnableMoves = new()
			{
				...
				new LearnableMove { MoveId = "harden", LearnLevel = 6 },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 8 },
				new LearnableMove { MoveId = "brace", LearnLevel = 10 },
				new LearnableMove { MoveId = "bramble_strike", LearnLevel = 12 },
				new LearnableMove { MoveId = "root_bind", LearnLevel = 15 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 17 },
				new LearnableMove { MoveId = "seed_volley", LearnLevel = 19 },
				new LearnableMove { MoveId = "sapling_stomp", LearnLevel = 21 },
				new LearnableMove { MoveId = "vine_crush", LearnLevel = 24 },
				new LearnableMove { MoveId = "tangling_roots", LearnLevel = 27 },
				new LearnableMove { MoveId = "pollen_burst", LearnLevel = 30 },
			},
			CardScale = 1.15f, CardOffsetX = -12f, CardOffsetY = 23f,
			MiniScale = 1.6f, MiniOffsetX = -13f, MiniOffsetY = 20f,
			DetailIconScale = 1.8f, DetailIconOffsetX = -21f, DetailIconOffsetY = 51f,
			BeastiaryNumber = 18

		} );

		// Skunkape — the elder bigfoot shepherd, mossy and leaf-crowned.
		// Florida-bigfoot cryptid subtext ("skunk ape") implicit; in-world
		// he reads as the elder of the Weaverwood, sovereign of the flock.
		AddSpecies( new MonsterSpecies
		{
			Id = "skunkape",
			Name = "Skunkape",
			Description = "The elder shepherd-giant of the Weaverwood, mossy and crowned in leaf-petals. He walks the village lanes at dusk, herding his Sheepots home. The children wave from the doorways, and he nods to each one in turn.",
			IconPath = "ui/monsters/skunkape/idle/skunkape_idle_01.png",
			ArtistCredit = "Jet",
			BaseHP = 100, BaseATK = 85, BaseDEF = 90, BaseSpA = 68, BaseSpD = 72, BaseSPD = 55,
			HPGrowth = 7, ATKGrowth = 6, DEFGrowth = 7, SpAGrowth = 6, SpDGrowth = 7, SPDGrowth = 7,
			Element = ElementType.Nature,
			BaseRarity = Rarity.Rare,
			EvolvesFrom = "stomplet",
			BaseCatchRate = 0.18f,
			BaseExpYield = 235,
			SignatureDropName = "Bigfoot Pelt",
			SignatureDropDescription = "A patch of mossy fur shed from an elder Skunkape. Smells like old earth and pine.",
			Personality = BeastPersonality.Loyal,
			PersonalityHint = "He greets you the way he greets the village children — a long look, a slow nod, and back to the flock.",
			PossibleTraits = new() { "verdant_power", "hardened_resolve", "menacing_aura" },
			LearnableMoves = new()
			{
				new LearnableMove { MoveId = "strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "bramble_strike", LearnLevel = 1 },
				new LearnableMove { MoveId = "harden", LearnLevel = 1 },
				new LearnableMove { MoveId = "nature_shield", LearnLevel = 1 },
				new LearnableMove { MoveId = "brace", LearnLevel = 1 },
				new LearnableMove { MoveId = "thorn_lash", LearnLevel = 5 },
				new LearnableMove { MoveId = "blade_leaf", LearnLevel = 8 },
				new LearnableMove { MoveId = "root_bind", LearnLevel = 12 },
				new LearnableMove { MoveId = "tangling_roots", LearnLevel = 15 },
				new LearnableMove { MoveId = "bloom_burst", LearnLevel = 20 },
				new LearnableMove { MoveId = "pollen_burst", LearnLevel = 25 },
				new LearnableMove { MoveId = "vine_crush", LearnLevel = 34 },
				new LearnableMove { MoveId = "verdant_edge", LearnLevel = 38 },
				new LearnableMove { MoveId = "overgrowth_slam", LearnLevel = 50 },
				new LearnableMove { MoveId = "shepherds_vigil", LearnLevel = 55 },
			},
			CardScale = 1.05f, CardOffsetX = -4f, CardOffsetY = 20f,
			MiniScale = 1.5f, MiniOffsetX = 2f, MiniOffsetY = 42f,
			DetailIconScale = 1.75f, DetailIconOffsetX = -1f, DetailIconOffsetY = 96f,
			BeastiaryNumber = 19
	...
	/// <summary>
	/// Pull owned-monsters + MaxMonsters out of the save blob. Runs once after
	/// <see cref="SaveService.OnSaveLoaded"/> fires.
	/// </summary>
	private void Hydrate()
	{
		...
		bool needsSave = false;
		// Defensive copy: the loop below prunes null-SpeciesId monsters, which
		// would otherwise throw downstream ("Value cannot be null. (Parameter
		// 'key')") in any dictionary lookup keyed by SpeciesId.
		var prunedMonsters = new List<Monster>( OwnedMonsters.Count );
		foreach ( var monster in OwnedMonsters )
		{
			// Guard against corrupted/mangled saves where SpeciesId round-tripped
			// as null. Skip the monster entirely rather than crash the render tree.
			if ( string.IsNullOrEmpty( monster.SpeciesId ) )
			{
				Log.Warning( $"[MonsterManager] Dropping monster with null/empty SpeciesId (Id={monster.Id}, Nickname={monster.Nickname})" );
				needsSave = true;
				continue;
			}
		...
		// Replace with pruned list if any monsters were dropped.
		if ( prunedMonsters.Count != OwnedMonsters.Count )
			OwnedMonsters = prunedMonsters;
		...
		Log.Info( $"[MonsterManager] Hydrated: {OwnedMonsters.Count} monsters, MaxMonsters={MaxMonsters}" );
	}
	...
	/// <summary>
	/// Push the in-memory monster roster + MaxMonsters back into the save blob
	/// and mark it dirty. Kept as <c>SaveMonsters</c> so existing call sites
	/// across the UI/ItemManager/BattleManager/etc. keep working.
	/// </summary>
	public void SaveMonsters()
	{
		...
		blob.Monsters = OwnedMonsters;
		blob.MaxMonsters = MaxMonsters;
		service.MarkDirty( "monsters" );
	}
	...
	// ─── DEV: GRANT A SPECIFIC BEAST ─────────────────────────────────
	// Usage: dev_givebeast <speciesId> [level]
	//   level defaults to 5 if omitted. Species id is the lowercase id
	//   from MonsterManager (e.g. wishlift, aurael, jackacabra).
	// `dev_listbeasts` dumps every known species id so you don't have to
	// dig through the C# source to look one up.
	[ConCmd( "dev_givebeast" )]
	public static void DevGiveBeast( string speciesId, int level = 5 )
	{
		if ( Instance == null ) { Log.Warning( "[dev_givebeast] MonsterManager not ready." ); return; }
		if ( string.IsNullOrWhiteSpace( speciesId ) ) { Log.Warning( "[dev_givebeast] Pass a species id (e.g. wishlift). Try dev_listbeasts." ); return; }

		var species = Instance.GetSpecies( speciesId.ToLowerInvariant() );
		if ( species == null )
		{
			Log.Warning( $"[dev_givebeast] No species '{speciesId}'. Run dev_listbeasts." );
			return;
		}

		var monster = Instance.CreateMonster( species.Id );
		if ( monster == null ) { Log.Warning( "[dev_givebeast] CreateMonster failed (storage full?)." ); return; }

		level = Math.Clamp( level, 1, Monster.MaxLevel );
		if ( monster.Level != level )
		{
			monster.Level = level;
			Instance.RecalculateStats( monster );
			monster.CurrentHP = monster.MaxHP;
			Instance.SaveMonsters();
		}
		Log.Info( $"[dev_givebeast] Granted {species.Name} (Lv {monster.Level}) — {monster.Nickname}" );
	}

	[ConCmd( "dev_listbeasts" )]
	public static void DevListBeasts()
	{
		if ( Instance == null ) { Log.Warning( "[dev_listbeasts] MonsterManager not ready." ); return; }
		var all = Instance.GetAllSpecies().OrderBy( s => s.BeastiaryNumber ).ToList();
		Log.Info( $"[dev_listbeasts] {all.Count} species:" );
		foreach ( var sp in all )
		{
			Log.Info( $"  #{sp.BeastiaryNumber:D3}  {sp.Id,-22}  {sp.Name}  [{sp.BaseRarity}]" );
		}
	}
	...
	public Monster AddMonster( Monster monster )
	{
		...
		// Species mastery — count this beast toward the "contract/own" milestones
		BeastiaryManager.Instance?.OnBeastContracted( monster.SpeciesId );

		// If this species has no further evolution (terminal form), flag the
		// max-evolve milestone. This also catches single-stage species that have
		// no evolution line at all — they ARE their own max form.
		if ( species != null && string.IsNullOrEmpty( species.EvolvesTo ) )
		{
			BeastiaryManager.Instance?.OnBeastMaxEvolved( monster.SpeciesId );
		}
	...
	public Monster BreedMonsters( Monster parent1, Monster parent2, HashSet<string> lockedGenes = null )
	{
		...
		if ( TamerManager.Instance?.CurrentTamer != null )
		{
			...
			Stats.SetValue( "monsters-bred-launch", TamerManager.Instance.CurrentTamer.TotalMonstersBred );
		...
		BeastiaryManager.Instance?.OnBeastFused( parent1.SpeciesId );
		if ( parent2.SpeciesId != parent1.SpeciesId )
			BeastiaryManager.Instance?.OnBeastFused( parent2.SpeciesId );
	...
	/// <summary>
	/// Apply the tamer's Beastbook species-mastery bonus (0-12%) to this monster's stats.
	/// Replaces the old per-instance veteran rank system.
	/// </summary>
	private void ApplySpeciesMasteryBonus( Monster monster )
	{
		float bonusPercent = BeastiaryManager.Instance?.GetMasteryBonus( monster.SpeciesId ) ?? 0f;
		if ( bonusPercent <= 0 )
			return;
	...
	public bool EvolveMonster( Guid monsterId )
	{
		...
		// Species mastery — treat the evolved form as newly "contracted" for its line
		// (mastery is per-species, so evolving restarts progress on the new SpeciesId).
		BeastiaryManager.Instance?.OnBeastContracted( evolvedSpecies.Id );

		// If the evolved species is a terminal form, fire the max-evolve milestone
		// for BOTH the evolved species (the one we're now in) AND retroactively for
		// the previous stage that the player just completed the chain on.
		if ( string.IsNullOrEmpty( evolvedSpecies.EvolvesTo ) )
		{
			BeastiaryManager.Instance?.OnBeastMaxEvolved( evolvedSpecies.Id );
			BeastiaryManager.Instance?.OnBeastMaxEvolved( fromSpecies.Id );
		}
		...
		if ( TamerManager.Instance?.CurrentTamer != null )
		{
			...
			Stats.SetValue( "monsters-evolved-launch", TamerManager.Instance.CurrentTamer.TotalMonstersEvolved );
		}
